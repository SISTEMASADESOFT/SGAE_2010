...
$(document).ready(function () {
    ...
    $('#MainContent_txtDesde').datepicker({ onSelect: function () {
      ...
      $('#MainContent_txtDesde').blur(function () {
     F_BuscarCuadreCaja();
    return false;
    ...
    $('#MainContent_btnLiquidacion').click(function () {
      ...
      $('#MainContent_btnCuadreCaja').click(function () {
        if (!F_SesionRedireccionar(AppSession)) return false;
        //if (F_PermisoOpcion(CodigoMenu, 7000304, '') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informacion
        try {
  ...
  if (!confirm("ESTA SEGURO DE APLICAR EL CAMBIO"))
    ...
    $('#MainContent_btnGrabar').click(function () {
     ...
     $('#MainContent_BtnGrabarCuadraCaja').click(function () {
        if (!F_SesionRedireccionar(AppSession)) return false;
        try {
    ...
    F_Derecha();
        ...
        $('#MainContent_btnGenerarCaja').prop('disabled', true);
...
function F_CuadreCaja() {
    
    try {

    

        $('#divCuadreCaja').dialog({
                    resizable: false,
                    modal: true,
                ...
                var TOTALSOLES = 0;
                    var MONEDA = 0;
                    var TOTALDOLARES = 0;
                    var PTOTALS = '#MainContent_grvCuadreCajaSoles_txtTotalSoles_';
                    var PTOTALD = '#MainContent_grvCuadreCajaDolares_txtTotalDolares_';
                    var PMONEDA = '#MainContent_grvConsulta_lblMoneda_';


                  if (F_PermisoOpcion_SinAviso(CodigoMenu, 7000305, '') === "0") {              
                    //SUMA
                    $('#MainContent_grvCuadreCajaSoles > tbody  > tr').each(function(index, tr) { 
                   
                       if (typeof $(PTOTALS +  (index )).val() !=='undefined'){
                        TOTALSOLES = TOTALSOLES+Number($(PTOTALS +  index).val());
                       }else{
                       TOTALSOLES = TOTALSOLES
                          ...
                          $('#MainContent_grvCuadreCajaDolares > tbody  > tr').each(function(index, tr) { 
                   
                        if (typeof $(PTOTALD +  (index )).val() !=='undefined'){
                        TOTALDOLARES = TOTALDOLARES+Number($(PTOTALD +  index).val());
                         }else{
                       TOTALDOLARES = TOTALDOLARES
                            ...
                            $('#MainContent_grvCuadreCajaSoles_txtTotalSoles_11').val(TOTALSOLES.toFixed(2));
                                $('#MainContent_grvCuadreCajaDolares_txtTotalDolares_6').val(TOTALDOLARES.toFixed(2));
                         if (TOTALSOLES.toFixed(2) >0 || TOTALDOLARES.toFixed(2)>0){
                       $('#divCuadreCaja').dialog('close');
                       alertify.log('NO TIENE PERMISO PARA EDITAR EL CUADRE DE CAJA');
                      }
...
function F_CuadreCajaEdicion(Fila) {
    
    try {
     if (F_PermisoOpcion_SinAviso(CodigoMenu, 7000305, '') === '0') {    
     alertify.log("NO TIENE PERMITIDO LA EDICION DE LA CAJA ");    
                     return false;
                     }
        ...
        $('#divCuadreCaja').dialog({
                ...
                $('#divCuadreCaja').dialog('open');
                 ...
                 F_BuscarCuadreCajaEdicion(Fila);
...
function F_BuscarCuadreCaja() {
    ...
    try {
        ...
        MostrarEspera(true);
        F_BuscarCuadreCaja_NET(arg, function (result) {
            ...
            if (str_resultado_operacion == "1") {
                ...
                F_Update_Division_HTML('div_CuadreCajaSoles', result.split('~')[2]);
                F_Update_Division_HTML('div_CuadreCajaDolares', result.split('~')[3]);
                $('#lblNumeroConsulta').text(F_Numerar_Grilla("grvCuadreCajaSoles", 'lblValorSoles')); 
                $('#lblNumeroConsulta').text(F_Numerar_Grilla("grvCuadreCajaDolares", 'lblValorDolares'));
                 $('.ccsestilo').css('background', '#FFFFE0');
                   ...
                   //cuadrecaja
                    ...
                    $('#MainContent_grvCuadreCajaSoles > tbody  > tr').each(function(index, tr) { 
                       ...
                       TOTALDOLARES = TOTALDOLARES
                        ...
                        $('#MainContent_btnGenerarCaja').prop('disabled', false);
                        $('#MainContent_btnReGenerarCaja').prop('disabled', false);
                         }else{
                      
                             $('#MainContent_btnGenerarCaja').prop('disabled', true);
                             $('#MainContent_btnReGenerarCaja').prop('disabled', true);
                              alertify.log('REALIZE PRIMERO EL CUADRE DE CAJA');
                      }

                     if (F_PermisoOpcion_SinAviso(CodigoMenu, 7000305, '') === '1') {        
                      $('#MainContent_btnGenerarCaja').prop('disabled', false);
                        $('#MainContent_btnReGenerarCaja').prop('disabled', false);
                     }
...
function F_BuscarCuadreCajaEdicion(Fila) {
var imgID = Fila.id;
 var Fecha = '#' + imgID.replace('imgCuadreCaja', 'lblFecha');

    try {
        var objParams = {
            Filtro_Desde: $(Fecha).text(),
            Filtro_CodAlmacen: $('#MainContent_ddlSucursal').val(),
            Filtro_ChkFecha: 1
        };
...
function SumaCuadreCajaSoles(fila) {
     

      var imgID = fila;

        var lblValorSoles = '#' + imgID.replace('txtCantidadSoles', 'lblValorSoles');
        var txtTotalSoles = '#' + imgID.replace('txtCantidadSoles', 'txtTotalSoles');
        var txttotal = '#' + imgID.replace('txtCantidadSoles', 'txtTotalSoles_11');
        var txtCantidadSolesSoles = '#' + imgID;
        var totalsoles = parseFloat ($(txtTotalSoles).val());

        $(txtTotalSoles).val(parseFloat($(lblValorSoles).text())*parseFloat($(txtCantidadSolesSoles).val()));
                    ...
                    var C = '0';
                    var F = '0';
                    var PTOTAL = '#MainContent_grvCuadreCajaSoles_txtTotalSoles_11';
                    var PVALOR= '#MainContent_grvCuadreCajaSoles_txtTotalSoles_';
                     var TOTALSOLES = 0;
                    ...
                    $('#MainContent_grvCuadreCajaSoles > tbody  > tr').each(function(index, tr) { 
                        ...
                        if ((PVALOR +  index !='#MainContent_grvCuadreCajaSoles_txtTotalSoles_11')&&(titulos ==true) )
                        {
                        TOTALSOLES = parseFloat(TOTALSOLES)+parseFloat($(PVALOR +  index).val());
                        }
                        else {
                        titulos=false
                        }
                    ...
                    $(PTOTAL).val(TOTALSOLES);
                  
}

function SumaCuadreCajaDolares(fila) {
     

      var imgID = fila;

        var lblValorDolares = '#' + imgID.replace('txtCantidadDolares', 'lblValorDolares');
        var txtTotalDolares = '#' + imgID.replace('txtCantidadDolares', 'txtTotalDolares');
        var txtCantidadDolares = '#' + imgID;

        $(txtTotalDolares).val(parseFloat($(lblValorDolares).text())*parseFloat($(txtCantidadDolares).val()));
                   ...
                   var titulos = true;
                    ...
                    var C = '0';
                    var F = '0';
                    var PTOTAL = '#MainContent_grvCuadreCajaDolares_txtTotalDolares_6';
                    var PVALOR= '#MainContent_grvCuadreCajaDolares_txtTotalDolares_';
                     var TOTALSOLES = 0;
                    ...
                    $('#MainContent_grvCuadreCajaDolares > tbody  > tr').each(function(index, tr) { 
                        ...
                        if ((PVALOR +  index !='#MainContent_grvCuadreCajaDolares_txtTotalDolares_6')&&(titulos ==true) )
                        {
                        TOTALSOLES = parseFloat(TOTALSOLES)+parseFloat($(PVALOR +  index).val());
                        }
                        else {
...
var arrSoles = new Array();
     var S=0;
     $('#MainContent_grvCuadreCajaSoles .detallesart').each(function () {
     lblValorSoles = '#MainContent_grvCuadreCajaSoles_lblValorSoles_'+S;
      txtCantidadSoles = lblValorSoles.replace('lblValorSoles', 'txtCantidadSoles');
      txtTotalSoles = lblValorSoles.replace('lblValorSoles', 'txtTotalSoles');
      hfCodCuadreCajaSoles = lblValorSoles.replace('lblValorSoles', 'hfCodCuadreCajaSoles');
      if ($(lblValorSoles).text() !='TOTAL'){
      var objSoles = {
                    CantidadSoles: $(txtCantidadSoles).val(),
                    ValorSoles: $(lblValorSoles).text(),
                    TotalSoles: $(txtTotalSoles).val(),
                    CodCuadreCajaSoles: $(hfCodCuadreCajaSoles).val()
                    
                };
                 arrSoles.push(objSoles);
            S++
        }
           
     });

     var arrDorales = new Array();
     var D=0;
     $('#MainContent_grvCuadreCajaDolares .detallesart').each(function () {
     lblValorDolares = '#MainContent_grvCuadreCajaDolares_lblValorDolares_'+D;
      txtCantidadDolares = lblValorDolares.replace('lblValorDolares', 'txtCantidadDolares');
      txtTotalDolares = lblValorDolares.replace('lblValorDolares', 'txtTotalDolares');
      hfCodCuadreCajaDolares = lblValorDolares.replace('lblValorDolares', 'hfCodCuadreCajaDolares');
      if ($(lblValorDolares).text() !='TOTAL'){
      var objDolares = {
                CantidadDolares: $(txtCantidadDolares).val(),
                    ValorDolares: $(lblValorDolares).text(),
                    TotalDolares: $(txtTotalDolares).val(),
                    CodCuadreCajaDolares: $(hfCodCuadreCajaDolares).val()
                    
                };
                 arrDorales.push(objDolares);
            D++
 }
    ...
    var objParams = {
                      Filtro_Fecha: $('#MainContent_txtFechaDesde').val(),
                      Filtro_CodAlmacen: $('#MainContent_ddlSucursal').val(),
                      Filtro_ChkFecha: 1,
                      Filtro_XmlSoles: Sys.Serialization.JavaScriptSerializer.serialize(arrSoles),
                      Filtro_XmlDolares: Sys.Serialization.JavaScriptSerializer.serialize(arrDorales)
                    };

    var arg = Sys.Serialization.JavaScriptSerializer.serialize(objParams);
    MostrarEspera(true);
    F_EdicionCuadreCaja_NET(arg, function (result) {
