var AppSession = "../Compras/RegistroFacturaCompraNuevoUnitario2025.aspx";
...
//    }
    ...
    $('#MainContent_txtClienteConsulta').autocomplete({
        ...
        $('#MainContent_btnListarOrdenes').click(function () {
    if (!F_SesionRedireccionar(AppSession)) return false;
    ...
    $('#MainContent_btnFacturarOrdenCompra').click(function () {
    if (!F_SesionRedireccionar(AppSession)) return false;
        try {
...
function F_FuncionesBotones() {
    ...
    k.down("enter", function (e) {
    e.preventDefault();
    e.stopPropagation();

    var control = $(document.activeElement);
    if (!control.length || !control.attr('id')) {
        console.warn("Sin control activo o sin ID");
        return true;
    }
...
function F_CamposAlternativas(Campos) {
                ...
                switch (Campos)
                {
                    ...
                    break;
                     ...
                     case "MainContent_txtOrdenCompra":
                        if ($("#MainContent_txtOrdenCompra").val().trim() != "")
                            {
                                if (isNaN($("#MainContent_txtOrdenCompra").val().trim()))
                                    $("#MainContent_txtOrdenCompra").val('');
                                else
                                    $('#MainContent_btnFacturarOrdenCompra').click();
                            }
                          break;
            ...
            (
                ...
                function (result) {
                    ...
                    if (str_resultado_operacion == "1") 
                    {
                        ...
                        F_Update_Division_HTML('div_OrdenCompra', result.split('~')[21]);
                        ...
                        $('#MainContent_txtOrdenCompra').css('background', '#FFFFE0');
                        $('#MainContent_ddlSerieOrdenComra').css('background', '#FFFFE0');
...
function F_AgregarTemporalCTxBoton(Boton) {
    $('#MainContent_txtOrdenCompra').val($('#COTIZACION_NUMERO_' + Boton.split('_')[2]).val());
    F_AgregarTemporalCTxNumero();
}
...
function F_AgregarTemporalCTxNumero() {
        ...
        if ($(Contenedor + 'txtOrdenCompra').val() == "")
            Mensaje = Mensaje + "\n" + "Numero Cotizacion";
    ...
    $.ajax({
       ...
       url: 'RegistroFacturaCompraNuevoUnitario.aspx/F_Obtener_Orden_PorNumero_Net',

        dataType: "json",
        data: JSON.stringify({
            NumeroCotizacion: $(Contenedor + 'txtOrdenCompra').val(),
             Seriedoc: $(Contenedor + 'ddlSerieOrdenComra option:selected').text().trim()}),
...
//            console.log("data:", data);
       ...
       var tasaigv = parseFloat($("#MainContent_ddlIgv option:selected").text()) + parseFloat(1);
                    ...
                    //Array de Items
                    $.each(data.OrdenCompraDet, function (index, item) { //lleno del detallle
                        var objDetalle = {
                            CodArticulo: item.CodArticulo,
                            Cantidad: item.Cantidad2,
                            CantidadOC: item.Cantidad2,
                            Precio: item.Precio / tasaigv,
                            PrecioDscto: item.Precio / tasaigv,
                            Precio2: item.Precio / tasaigv,
                            ...
                            Dscto: 0,
                            CodDetalle: 0,
                            CodOrdenCompraDet: item.CodDetalle,
                 ...
                 //   console.table(arrDetalle);

                    var objParams = {              //aca la cabecera de lo que devuelvo 
                        Filtro_CodTipoDoc: 5,
                        Filtro_SerieDoc: data.Serie,
                        Filtro_NumeroDoc: data.NumeroDoc,
                        ...
                        Filtro_CodProforma: data.CodOrdenCompra,
                        Filtro_Igv: data.Igv,
                ...
                if (result.split('~')[2] == "Los Producto(s) se han agregado con exito") 
                {
                    ...
                    $('#MainContent_txtProveedor').val(data.Cliente);  
            ...
            catch (x) { alertify.log('No Se Encontro Informacion'); }
            MostrarEspera(false);
        },
...
//                $.each(data.OrdenCompraDet, function (index, item) {
//                    var objDetalle = {
//                            CodArticulo: item.CodArticulo,
//                            Cantidad: item.Cantidad2,
//                            CantidadOC: item.Cantidad2,
...
//                F_AgregarTemporal_NET(arg, function (result) {
//                    var estado = result.split('~')[0];
//                    var mensaje = result.split('~')[1];
//                    MostrarEspera(false);

//                    if (estado == "1") {
//                        $('#MainContent_hfCodigoTemporal').val(result.split('~')[3]);
//                        F_Update_Division_HTML('div_grvDetalleArticulo', result.split('~')[4]);
...
//                      
//                        $('#MainContent_txtNroRuc').val(data.NroRuc);
//                        $('#MainContent_hfNroRuc').val(data.NroRuc);
//                        $('#MainContent_txtProveedor').val(data.Cliente);
//                        $('#MainContent_hfCliente').val(data.Cliente);
//                        $('#MainContent_hfCodCtaCte').val(data.CodCtaCte);
//                        $('#MainContent_txtDistrito').val(data.Distrito);
//                        $('#MainContent_txtDireccion').val(data.Direccion);

//                      
//                        $('#MainContent_ddlDireccion').empty();
//                        $('#MainContent_ddlDireccion').append(
//                            $("<option></option>").val(data.CodDireccion).html(data.Direccion)
//                        );

//                        alertify.success('Orden de Compra agregada con Exito');
//                    } else {
//                        alertify.error(mensaje);
//                    }
//                });
//            } catch (x) {
//                alertify.error("Error al procesar: " + x.message);
//            }
//        },
//        error: function (xhr, ajaxOptions, thrownError) {
//            alertify.error("Error AJAX: " + thrownError);
//        }
//    });
//}
...
var Subtotal=0;
            ...
            else
            {
                ...
                $(Cuerpo + 'txtIgv').val((Total*parseFloat($("#MainContent_ddlIgv option:selected").text())).toFixed(2));
                $(Cuerpo + 'txtMonto').val((Total*(1+parseFloat($("#MainContent_ddlIgv option:selected").text()))).toFixed(2));   
...
function F_ValidarGrabarDocumento(){
    try 
        ...
        if ($('#lblCantidadRegistro').text()=='0')
                Cadena=Cadena + '<p></p>' + 'NO INGRESADO NINGUN PRODUCTO';
                  ...
                  if ($(chkSi).is(':checked')) 
                    {
                        ...
                        Dscto: 0,
                        CodDetalle: $(hfCodDetalle).val(),
                        CodOrdenCompraDet: $(hfCodDetalle).val(),
                        OC: $(lblNumero).text(),
 ...
 try 
        {
            ...
            var txtPrecioFinal = txtPrecio.replace('txtPrecio', 'txtPrecioFinal');
            var txtPrecioMecanico = txtPrecio.replace('txtPrecio', 'txtPrecioMecanico');
            var tasaigv;
            var FlagIgv = 0;
            var Contenedor = '#MainContent_';    
         ...
         return false;
            ...
            var objParams = {
                              Filtro_Precio: parseFloat($(txtPrecio).val())/tasaigv,
                             ...
                             Filtro_PrecioFinal: $(txtPrecioFinal).val(),///tasaigv,
                             Filtro_PrecioMecanico: $(txtPrecioMecanico).val(), 
            };
...
function F_ActualizarPrecioFinal(Fila) {
    try {
        var txtPrecioFinal = '#' + Fila;
        var Clave = "";
        var hfCodDetalle = txtPrecioFinal.replace('txtPrecioFinal', 'hfCodDetalle');
        var hfPrecio = txtPrecioFinal.replace('txtPrecioFinal', 'hfPrecio');
        var hfCantidad = txtPrecioFinal.replace('txtPrecioFinal', 'hfCantidad');
        var txtPrecio = txtPrecioFinal.replace('txtPrecioFinal', 'txtPrecio');
        var txtClaveGrilla = txtPrecioFinal.replace('txtPrecioFinal', 'txtClaveGrilla');
        var hfDescripcion = txtPrecioFinal.replace('txtPrecioFinal', 'hfDescripcion');
        var txtCantidad = txtPrecioFinal.replace('txtPrecioFinal', 'txtCantidad');
        var txtDescripcion = txtPrecioFinal.replace('txtPrecioFinal', 'txtDescripcion');
       // var txtPrecioFinal = txtPrecioMecanico.replace('txtPrecioMecanico', 'txtPrecioFinal');
        var txtPrecioMecanico = txtPrecioFinal.replace('txtPrecioFinal', 'txtPrecioMecanico');
        var tasaigv = 1;
        var FlagIgv = 0;
        ...
        var objParams = {
            ...
            Filtro_CodMoneda: $('#MainContent_ddlMoneda').val() ,
            Filtro_PrecioFinal: $(txtPrecioFinal).val(), //tasaigv,
            Filtro_PrecioMecanico: $(txtPrecioMecanico).val(),//tasaigv,       
        };
...
function F_ActualizarPrecioMecanico(Fila) {
    try {
        var txtPrecioMecanico = '#' + Fila;
        var Clave = "";
        var hfCodDetalle = txtPrecioMecanico.replace('txtPrecioMecanico', 'hfCodDetalle');
        var hfPrecio = txtPrecioMecanico.replace('txtPrecioMecanico', 'hfPrecio');
        var hfCantidad = txtPrecioMecanico.replace('txtPrecioMecanico', 'hfCantidad');
        var txtPrecio = txtPrecioMecanico.replace('txtPrecioMecanico', 'txtPrecio');
        var txtClaveGrilla = txtPrecioMecanico.replace('txtPrecioMecanico', 'txtClaveGrilla');
        var hfDescripcion = txtPrecioMecanico.replace('txtPrecioMecanico', 'hfDescripcion');
        var txtCantidad = txtPrecioMecanico.replace('txtPrecioMecanico', 'txtCantidad');
        var txtDescripcion = txtPrecioMecanico.replace('txtPrecioMecanico', 'txtDescripcion');
        var txtPrecioFinal = txtPrecioMecanico.replace('txtPrecioMecanico', 'txtPrecioFinal');
        var tasaigv = 1;
        var FlagIgv = 0;
 ...
 try 
        {
            ...
            var txtPrecioFinal = txtCantidad.replace('txtCantidad', 'txtPrecioFinal');
            var txtPrecioMecanico = txtCantidad.replace('txtCantidad', 'txtPrecioMecanico');
            var tasaigv;
            var FlagIgv = 0;
            var Contenedor = '#MainContent_';    
            if(parseFloat($(txtPrecio).val())==parseFloat($(hfPrecio).val()) & parseFloat($(txtCantidad).val())==parseFloat($(hfCantidad).val()))
                  ...
                  if (str_mensaje_operacion == "Se actualizó con éxito")
                   {
                    ...
                    F_Update_Division_HTML('div_grvDetalleArticulo', result.split('~')[4]);
                    $('#lblCantidadRegistro').text(F_Numerar_Grilla2("grvDetalleArticulo", "lblProducto"));
                     $('.ccsestilo').css('background', '#FFFFE0');
                   }
                else 
                {
...
function F_ActualizarDescripcion(Fila) {
    try {
        ...
        var txtPrecioFinal = txtDescripcion.replace('txtDescripcion', 'txtPrecioFinal');
        var txtPrecioMecanico = txtDescripcion.replace('txtDescripcion', 'txtPrecioMecanico');
        var tasaigv = 1;
        var FlagIgv = 0;
...
function F_Pedidos() {
    ...
    $.ajax({
        ...
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        url: 'RegistroFacturaCompraNuevoUnitario.aspx/F_Consulta_Ordenes_Pendientes_NET',
        dataType: "json",
      //  data: "{'CodAlmacen':'" + $('#hfCodSede').val()  + "'}",
         data: JSON.stringify({ CodAlmacen: parseInt($('#hfCodSede').val()) }),
        success: function (dbObject) {
            MostrarEspera(true);
            var data = dbObject.d;
            try {
                ...
                $.each(data.rows, function (index, item) {
                    var fmt2 =   ' <li style="width: 220px; height:30px; padding-top:0px; display:block"> <div class="linea-button"> ' +
								...
								'  		<td style="white-space: nowrap; width: 100px; padding-left:45px;">' +
                    ...
                    var li = fmt2.replace('@Nro', Count).replace('@ID', 'COTIZACION_CODPROFORMA_' + Count).replace('@CodProforma', item.CodProforma)
                                        .replace('@NroDocumento', String(item.Serie).padStart(4, '0') + '-' + Number(item.NumeroC)).replace("@ElVendedor", item.Vendedor).replace('@Cliente', item.Cliente)
                                        .replace('@ID_SERIE', 'COTIZACION_SERIE_' + Count).replace('@Serie', item.Serie)
                                        .replace('@ID_NUMERO', 'COTIZACION_NUMERO_' + Count).replace('@Numero', item.NumeroC)
