                    ...
                    using (var package = new ExcelPackage(fileInfo))
                    {
                        ...
                        using (SqlConnection connection = new SqlConnection(sqlConnectionString))
                        {
                            ...
                            for (int row = worksheet.Dimension.Start.Row + 1; row <= worksheet.Dimension.End.Row; row++)
                            {
                                ...
                                string P2 = worksheet.Cells[row, 6].Text;
                                string P3 = worksheet.Cells[row, 7].Text;
                                string IDFamilia = worksheet.Cells[row, 8].Text;
                                string Limite = worksheet.Cells[row, 9].Text;
                                string Exclusivo = worksheet.Cells[row, 10].Text;

                                // Realizar la inserción en la base de datos
                                string insertQuery = "INSERT ImportacionExcel(CodigoInterno,Descripcion,Cantidad,Costo,P1,P2,P3,IDFamilia,Limite,Exclusivo,IDPruebasExcelCab,NombreArchivo) " +
                                                     "VALUES (@CodigoInterno,@Descripcion,@Cantidad,@Costo,@P1,@P2,@P3,@IDFamilia,@Limite,@Exclusivo,@IDPruebasExcelCab,@NombreArchivo)";
