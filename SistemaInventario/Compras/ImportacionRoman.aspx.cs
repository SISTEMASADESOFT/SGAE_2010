...
{
    public partial class ImportacionRoman : System.Web.UI.Page
    {
    ...
    ref DropDownList ddl_combodocumento, ref DropDownList ddl_comboclasificacion)
        ...
        public void P_AnularRegistro(Hashtable objTablaFiltro, ref String Mensaje)
        {
            ...
            objOperacion.F_Anulacion_NotaIngreso_Importacion(objEntidad);
                    ...
                    using (var package = new ExcelPackage(fileInfo))
                    {
                        ...
                        using (SqlConnection connection = new SqlConnection(sqlConnectionString))
                        {
                            ...
                            for (int row = worksheet.Dimension.Start.Row + 1; row <= worksheet.Dimension.End.Row; row++)
                            {
                                ...
                                string Cantidad = worksheet.Cells[row, 3].Text;
                                string Costo = worksheet.Cells[row, 4].Text;
                                string P1 = worksheet.Cells[row, 5].Text;
                                string Margen = worksheet.Cells[row, 6].Text;
                                string Marca = worksheet.Cells[row, 7].Text;
                                string Pais = worksheet.Cells[row, 8].Text;
                                string Modelo = worksheet.Cells[row, 9].Text;
                                string Medida = worksheet.Cells[row, 10].Text;
                                string Posicion = worksheet.Cells[row, 11].Text;
                                string Familia = worksheet.Cells[row, 12].Text;

                                // Realizar la inserción en la base de datos
                                string insertQuery = "INSERT ImportacionExcel(CodigoInterno,Descripcion,Cantidad,Costo,P1,Margen,Marca,Pais,Modelo,Medida,Posicion,Familia,IDPruebasExcelCab,NombreArchivo) " +
                                                     "VALUES (@CodigoInterno,@Descripcion,@Cantidad,@Costo,@P1,@Margen,@Marca,@Pais,@Modelo,@Medida,@Posicion,@Familia,@IDPruebasExcelCab,@NombreArchivo)";
