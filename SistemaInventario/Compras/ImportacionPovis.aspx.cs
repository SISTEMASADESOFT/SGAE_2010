...
{
    public partial class ImportacionPovis : System.Web.UI.Page
    {
                    ...
                    using (var package = new ExcelPackage(fileInfo))
                    {
                        ...
                        using (SqlConnection connection = new SqlConnection(sqlConnectionString))
                        {
                            ...
                            for (int row = worksheet.Dimension.Start.Row + 1; row <= worksheet.Dimension.End.Row; row++)
                            {
                                ...
                                string Cantidad = worksheet.Cells[row, 5].Text;
                                string Costo = worksheet.Cells[row, 6].Text;
                                string PL = worksheet.Cells[row, 7].Text;
                                string PMIN = worksheet.Cells[row, 8].Text;
                                string PF = worksheet.Cells[row, 9].Text;
                                string PC = worksheet.Cells[row, 10].Text;
                                string IDFamilia = worksheet.Cells[row, 11].Text;

                                // Realizar la inserción en la base de datos
                                string insertQuery = "INSERT ImportacionExcel(CodigoInterno,Descripcion,Marca,Categoria,Cantidad,Costo,PL,P1,P2,P3,IDFamilia,IDPruebasExcelCab,NombreArchivo) " +
                                                     "VALUES (@CodigoInterno,@Descripcion,@Marca,@Categoria,@Cantidad,@Costo,@PL,@PMIN,@PF,@PC,@IDFamilia,@IDPruebasExcelCab,@NombreArchivo)";
