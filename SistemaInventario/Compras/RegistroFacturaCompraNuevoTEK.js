    ...
    if (F_PermisoOpcion(CodigoMenu, CodigoInterno, 'Insertar') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informacion
     try 
        ...
        catch (e) 
        {
          ...
          $('#MainContent_btnGrabarDistribucion').click(function () {
    if (!F_SesionRedireccionar(AppSession)) return false;
    if (F_PermisoOpcion(CodigoMenu, CodigoInterno, 'Insertar') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informacion
     try 
        {
 
            if (confirm("ESTA SEGURO DE GRABAR LA DISTRIBUCION"))
            F_GrabarDocumentoDistribucion();
...
if (F_PermisoOpcion(CodigoMenu, CodigoInterno, 'Editar') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informacion
        try {
            ...
            if ($(lblEstado).text()=='CANCELADO')
            {
               alertify.log("LA FACTURA " + $(lblNumero).text() + " NO SE PUEDE MODIFICAR, SE ENCUENTRA CANCELADA");
               return false;
            }
...
function F_distribucion(Fila) {
       
       var imgID = Fila.id;

        var hfcoddetdetalle = '#' + imgID.replace('imgEditarRegistro', 'hfCodDetalle');
        var hfCodArticulo = '#' + imgID.replace('imgEditarRegistro', 'hfCodArticulo');
        var hfCantidad = '#' + imgID.replace('imgEditarRegistro', 'hfCantidad');


        try {
            var objParams = {
                Filtro_coddetdetalle:$(hfcoddetdetalle).val(),
                Filtro_CodArticulo:$(hfCodArticulo).val(),
                Filtro_Cantidad:$(hfCantidad).val()
            };

            var arg = Sys.Serialization.JavaScriptSerializer.serialize(objParams);
            MostrarEspera(true);

            F_Distribucion_NET(arg, function (result) {
                MostrarEspera(false);
                ...
                if (str_resultado_operacion == "1")
                {
                      ...
                      $("#div_Distribucion").dialog({
            resizable: false,
            modal: true,
            title: "Distribucion Almacen",
            title_html: true,
            height: 200,
            width: 500,
            autoOpen: false
        });
                        F_Update_Division_HTML('div_DetalleDostribucion', result.split('~')[2]);
                        
                $('#hfCodDistribucion').val(result.split('~')[3]);
                          $('.ccsestilo').css('background', '#FFFFE0');
                        if (str_mensaje_operacion!="")
                        alertify.log(str_mensaje_operacion);
                        else
                        $('#div_Distribucion').dialog('open');
    ...
    function F_GrabarDocumentoDistribucion(){
  try 
        {       
      var F=0;
      var arrDetalle = new Array();

        $('#MainContent_grvDetalleDistribucion .ccsestilo').each(function () {
                    CodAlmacen = '#MainContent_grvDetalleDistribucion_hfCodAlmacen_'+F;
                    hfCantidadTotal = CodAlmacen.replace('hfCodAlmacen', 'hfCantidadTotal');
                    hfCodarticulo = CodAlmacen.replace('hfCodAlmacen', 'hfCodarticulo');
                    hfCodDetalleDocumento = CodAlmacen.replace('hfCodAlmacen', 'hfCodDetalleDocumento');
                    txtCantidadDistribuida = CodAlmacen.replace('hfCodAlmacen', 'txtCantidadDistribuida');
                    hfCodDetDistribucion = CodAlmacen.replace('hfCodAlmacen', 'hfCodDetDistribucion');

                    var objDetalle = {
                    Codalmacen: $(CodAlmacen).val(),
                    CantidadTotal: $(hfCantidadTotal).val(),
                    CodArticulo: $(hfCodarticulo).val(),
                    CoddetalleDocumento: $(hfCodDetalleDocumento).val(),
                    CantidadDistruida: $(txtCantidadDistribuida).val(),
                    CodDetDistribucion: $(hfCodDetDistribucion).val()
                ...
                var objParams = {
                      Filtro_Coddistribucion:$('#hfCodDistribucion').val(),  
                      Filtro_XmlDetalle: Sys.Serialization.JavaScriptSerializer.serialize(arrDetalle)
                               };
                ...
                MostrarEspera(true);
                F_GrabarDocumentoDistribucion_NET(arg, function (result) {
                ...
                {                  
                    ...
                    {
                        alertify.log('SE GRABO LA DISTRIBUCION CORRECTAMENTE');
                         $('#div_Distribucion').dialog('close');
                    }
                    else
                    {
                        alertify.log(str_mensaje_operacion);
