    ...
    public class Stocks : System.Web.Services.WebService
    {
        ...
        public csDescarga DescargarDocumentosSunat(int CodDocumentoVenta, string TipoDocumento)
        {
            ...
            if (dtDatos.Rows.Count > 0)
            {
                ...
                F_RutaEmpresa_Respuesta rpta = new F_RutaEmpresa_Respuesta();
        ...
        [WebMethod]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public csDescarga DescargarDocumentosSunatGrupal(string TipoDocumento, string Conexion, int Codigo)
        {
            csDescarga datos = new csDescarga();
            ...
            DataTable dtDatos = (new TCEmpresaCN()).F_DatosDocumento_Descarga_Grupal(objEntidad);

            for (int i = 0; i < dtDatos.Rows.Count; i++)
            {
                CodDocumentoVenta = Convert.ToInt32(dtDatos.Rows[i]["CodDocumentoVenta"].ToString());
                RucEmisor = Convert.ToInt64(dtDatos.Rows[i]["RucEmisor"].ToString());
                RucReceptor = Convert.ToInt64(dtDatos.Rows[i]["RucReceptor"].ToString());
                Tipo = dtDatos.Rows[i]["Tipo"].ToString();
                Serie = dtDatos.Rows[i]["Serie"].ToString();
                Numero = dtDatos.Rows[i]["Numero"].ToString();
                Fecha = dtDatos.Rows[i]["Fecha"].ToString();
                //Guia = dtDatos.Rows[i]["Guia"].ToString();
                CodEmpresa = Convert.ToInt32(dtDatos.Rows[i]["CodEmpresa"].ToString());

                F_RutaEmpresa_Respuesta rpta = new F_RutaEmpresa_Respuesta(CodEmpresa, Conexion);
                datos.Mensaje = rpta.MsgError;
                if (rpta.MsgError == null & rpta.Ruta != null)
                {
        ...
        public class F_RutaEmpresa_Respuesta
        {
            ...
            public F_RutaEmpresa_Respuesta(int CodEmprea, String Conexion)
            {
                try
                {
                    this.Ruta = (new TCEmpresaCN()).RutaFacturadorPorCodEmpresa(CodEmprea, Conexion);
                }
                catch (Exception ex)
                {
