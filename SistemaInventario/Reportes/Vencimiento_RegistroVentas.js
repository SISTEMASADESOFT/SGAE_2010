var AppSession = "../Reportes/Vencimiento_RegistroVentas.aspx";
var ddlCliente;
...
function F_PDF(){
    ...
    var CodVendedor = 0;
    var pdf = '#' + Fila;
    var Desde = pdf.replace('txtCantidad', 'hfFechaMin'); 
    var Hasta = pdf.replace('txtCantidad', 'hfFechaMax'); 
    ...
    if ($('#MainContent_chkRango').is(':checked')) {
        Titulo = "REPORTE DE COBRANZAS DESDE " + $('#MainContent_txtDesde').val()+" HASTA "  + $('#MainContent_txtHasta').val();
        CodMenu = 400;
    }
    ...
    rptURL = rptURL + 'Desde='          + $(Desde).val()  + '&';
    rptURL = rptURL + 'Hasta='          + $(Hasta).val()  + '&';
...
//    });
    ...
    try {
        var objParams = {
            ...
            Filtro_CodMoneda: $('#MainContent_ddlMarca').val(),
            Filtro_CodVendedor: $('#MainContent_ddlVendedor').val(),
            Filtro_Cliente: Sys.Serialization.JavaScriptSerializer.serialize(arrClientes)
        };
