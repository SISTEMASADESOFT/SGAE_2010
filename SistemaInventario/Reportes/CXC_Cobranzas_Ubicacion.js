var AppSession = "../Reportes/CXC_Cobranzas_Ubicacion.aspx";
var ddlCliente;
...
function F_Buscar(){
    ...
    var sucursales = $('#MainContent_ddlSucursales').multipleSelect('getSelects');
    var Filtro_Sucursales = sucursales.join(',');
    ...
    if (ddlCliente.selected().length==1)
    {
       ...
       NombreArchivo = "Web_Reporte_Cobranzas_rptCobranzasUnitario_Edificio.rpt";
    }
    ...
    rptURL = rptURL + 'CodDepartamento='    + $("#MainContent_ddlDepartamento").val() + '&';
    rptURL = rptURL + 'NombreTabla='    + NombreTabla + '&';
    rptURL = rptURL + 'NombreArchivo='  + NombreArchivo + '&';

    rptURL = rptURL + 'Filtro_Sucursales=' + Filtro_Sucursales + '&';
            ...
            (
                ...
                function (result) {
                    ...
                    if (str_resultado_operacion == "1") {
                        ...
                        F_Update_Division_HTML('div_Departamento', result.split('~')[6]);
                        F_Update_Division_HTML('div_Sucursales', result.split('~')[7]);
                        F_InicializarSucursal();
                        $('#MainContent_ddlEmpresa').val(0);
                        $('#MainContent_ddlRuta').val(0);
                        $('#MainContent_ddlVendedor').val(0);
                        $('#MainContent_ddlMoneda').val(1);
                        $('#MainContent_ddlDepartamento').val(0);
                        $('#MainContent_ddlVendedor').css('background', '#FFFFE0');
                        $('#MainContent_ddlSucursales').css('background', '#FFFFE0');
                        $('#MainContent_ddlEmpresa').css('background', '#FFFFE0');
                        $('#MainContent_ddlRuta').css('background', '#FFFFE0');
                        $('#MainContent_ddlMoneda').css('background', '#FFFFE0');
                        $('#MainContent_ddlDepartamento').css('background', '#FFFFE0');
                        $("#MainContent_chkClienteSumarizado").prop("checked", true);
...
function F_InicializarSucursal() {
    var $ddlSucursal = $("[id$='ddlSucursales']");
    
    if ($ddlSucursal.data('multipleSelect')) {
        $ddlSucursal.multipleSelect('destroy');
    }

    $ddlSucursal.multipleSelect({
        selectAll: true,
        single: false,
        width: 300,
        placeholder: "Seleccione sucursal",
        allSelected: "Todas las sucursales"
    });

    if (F_PermisoOpcion_SinAviso(10000,10050, '') === "0") {
        $ddlSucursal.multipleSelect('disable');
        
        $ddlSucursal.next('.ms-parent')
            .find('.ms-choice')
            .css('background', '#FFFFE0');

    } else {
        $ddlSucursal.multipleSelect('enable');
    }


 //   $ddlSucursal.multipleSelect('checkAll');
...
function F_Excel() {
    ...
    var cantidad = ddlCliente.selected().length;
    var Columna = 12; // A PARTIR DE Q COLUMNA SE VA ELIMINAR PARA Q SE VISUALICE COREECTAMENTE EL EXCEL
    ...
    rptURL = rptURL + 'NombreArchivo='  + NombreArchivo + '&';
    rptURL = rptURL + 'Sumarizado='     + Sumarizado + '&';
