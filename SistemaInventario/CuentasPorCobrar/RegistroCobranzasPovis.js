var AppSession = "../CuentasPorCobrar/RegistroCobranzasPovis.aspx";
...
$(document).ready(function () {
    ...
    if (F_PermisoOpcion(CodigoMenu, 4001802, '') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informac
     try 
        {
          F_BuscarDetallado();
          return false;
...
//    }
    ...
    $('#MainContent_txtClienteDetallado').css('background', '#FFFFE0');
     ...
     $('#MainContent_txtRet').css('background', '#FFFFE0');
...
function F_LlenarGridDetalle_Detallado(Fila){
  try 
        {             
                ...
                var TipoDoc = $('#' + Fila.replace('pnlDetallado', 'hfCodtipoDoc')).val();
       ...
       try 
        {
              ...
              if ($('#MainContent_chkRet').is(':checked'))
              chkRet='1';
              ...
              var objParams = {
                                 ...
                                 Filtro_ChkFecha: chkFecha,
                                 Filtro_ChkCliente: chkCliente,
                                 Filtro_ChkRet: chkRet,
                                 Filtro_FlagAcuenta: $("#MainContent_ddlFlagAcuenta").val(),
                                 Filtro_Ret: $('#MainContent_txtRet').val(),
...
//                    if ($('#MainContent_chkRet').is(':checked'))
//                    Retraso= $('#MainContent_txtRet').val();
//              
//              rptURL = '../Reportes/Web_Pagina_Crystal_Nuevo.aspx';
//              rptURL = rptURL + '?';
//              rptURL = rptURL + 'CodMenu='        + CodMenu + '&';
...
function F_Excel() {
              ...
              var SubTitulo = "DESDE " + $("#MainContent_txtDesdeDetallado").val() + " HASTA " + $('#MainContent_txtHastaDetallado').val();
              var NombreTabla = "DetalladoCobranzaPovis";
              var NombreArchivo = "Xls_ReportexMesxCobranza.xlsx";
              ...
              rptURL = '../Reportes/Web_Pagina_ConstruirExcel_antiguo.aspx';
              rptURL = rptURL + '?';
              rptURL = rptURL + 'CodMenu='        + CodMenu + '&';
              rptURL = rptURL + 'CodTipoArchivo=' + CodTipoArchivo + '&';
              rptURL = rptURL + 'TipoArchivo='    + TipoArchivo + '&';
...
function imgMas_Click_Letras(Control) {
    CtlgvVentas = Control;
    var Src = $(Control).attr('src');
    ...
    if (Src.indexOf('plus') >= 0) {
        ...
        var grid = $(Control).next();
        F_LlenarGridDetalle_Letra(grid.attr('id'));
    }
    else {
        $(Control).attr("src", "../Asset/images/plus.gif");
        $(Control).closest("tr").next().remove();
...
function F_LlenarGridDetalle_Letra(Fila){
  try 
        {             
                var nmrow = 'MainContent_grvDetallado_pnlDetalleLetra_0';
                var Col = Fila.split('_')[3];
                var Codigo = $('#' + Fila.replace('pnlDetalleLetra', 'hfCodDocumentoVenta')).val();
                var CodTipoDoc = $('#' + Fila.replace('pnlDetalleLetra', 'hfCodtipoDoc')).val();
                var grvNombre = 'MainContent_grvDetallado_grvDetalleLetra_' + Col;
                HfgvVentas = '#' + Fila.replace('pnlDetalleFactura', 'hfVentasCargado');
                ...
                else 
                {
                        ...
                        $(CtlgvVentas).attr('src', '../Asset/images/loading.gif');
                        F_LlenarGridDetalleLetra_NET(arg, function (result) {
