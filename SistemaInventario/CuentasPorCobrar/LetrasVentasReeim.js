var AppSession = "../CuentasPorCobrar/LetrasVentasReeim.aspx";
...
$(document).ready(function () {
    ...
    $('#MainContent_txtPrimerVencimiento').on('change', function (e) {
        F_Cronograma();
        F_TipoCambio();
    });
...
$(document).on("change", "select[id $= 'MainContent_ddlFormaPago']",function () {
 ...
 for (var i = 1, l = parseInt($('#MainContent_txtCantidadLetra').val())+1 ; i < l; i++) {
         if (i==1){
             $('#MainContent_txtVencimiento' + i).val($('#MainContent_txtPrimerVencimiento').val());
         }else {
                 $('#MainContent_txtVencimiento' + i).val(Date_AddDays($('#MainContent_txtPrimerVencimiento').val(), parseInt($('#MainContent_txtIntervalo').val() * (i-1))));
             }
     }
  ...
  try 
        {
            ...
            var hfCodTipoDoc = '';
               ...
               var objParams = {
                                        ...
                                        Filtro_Vencimiento: $(Contenedor + 'txtPrimerVencimiento').val(),
...
function F_ImprimirFacturaHTML(Fila, rplc, TipoImp, TipoDoc) {
    ...
    var imgID = Fila.id;
    var CantidadDocumentos = '#' + imgID.replace(rplc, 'hfCantidadDocumentos');
...
//    var lblNumero = '#' + imgID.replace(rplc, 'hfNumeroDocumento');
    ...
    var Codigos = '#' + imgID.replace(rplc, 'hfcodletras');

    if (Number($(CantidadDocumentos).val()) === 0) {
        toastr.error("NO TIENE DOCUMENTOS A MOSTRAR");
        return false;
    }
    ...
    $.ajax({
        ...
        data: "{'Codigos':'" + $(Codigos).val() + "','CodTipoDoc':'" + $(CodTipoDoc).val() + "'}",
        dataType: "json",
        async: true,
        success: function (data) {
