...
{
    public partial class RegistroAjusteExcelRoman : System.Web.UI.Page
    {
            ...
            if (FileUpload1.HasFile)
            {
                if (FileUpload1.PostedFile.ContentType == ExcelContentType || FileUpload1.PostedFile.ContentType == Excel2010ContentType)
                {
                    ...
                    try
                    {
                        ...
                        Cadena = string.Format("Select [Codigo],[StockInvent]," + HiddenField1.Value + " AS IDPruebasExcelCab,'" + FileUpload1.FileName + "' AS NombreArchivo, " + Convert.ToInt32(Session["CodSede"]).ToString() + " as CodAlmacen FROM [{0}] ", "Inventario$");
                        ...
                        using (OleDbConnection connection = new OleDbConnection(excelConnectionString))
                        {
                            ...
                            using (DbDataReader dr = command.ExecuteReader())
                            {
                                ...
                                using (SqlBulkCopy bulkCopy = new SqlBulkCopy(sqlConnectionString))
                                {
                                    ...
                                    bulkCopy.ColumnMappings.Add("[Codigo]", "CodProducto");
                                    bulkCopy.ColumnMappings.Add("[StockInvent]", "Cantidad");
