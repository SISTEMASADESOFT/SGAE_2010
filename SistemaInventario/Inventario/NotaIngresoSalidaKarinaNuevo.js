var AppSession = "../Inventario/NotaIngresoSalidaKarinaNuevo.aspx";
...
$(document).ready(function () {
    ...
    $('#MainContent_btnAnular').click(function () {
        ...
        $('#MainContent_btnConfirmacion').click(function () {
        if (!F_SesionRedireccionar(AppSession)) return false;
            try {
                if ($.trim($('#MainContent_txtObservacionConfirmacion').val()).length<10)
                {
                 toastr.warning("INGRESAR LA OBSERVACION (MINIMO 10 CARACTERES)");
                  return false;
                }
            ...
            (
                ...
                function (result) {
                    ...
                    if (str_resultado_operacion == "1") 
                    {
                        ...
                        F_Update_Division_HTML('div_Empleado', result.split('~')[23]);
                        F_Update_Division_HTML('div_Estado', result.split('~')[24]);
...
function F_ValidarGrabarDocumento(){
    try 
        ...
        if ($.trim($(Cuerpo + 'txtSerie').val())=='' & $(Cuerpo + 'ddlTipoDocumento').val()==10)
            Cadena=Cadena + '<p></p>' + 'SERIE';

        if ($.trim($(Cuerpo + 'txtNumero').val())=='' & $(Cuerpo + 'ddlTipoDocumento').val()==10)
        ...
        if ($('#MainContent_ddlDestino').val() == null)
            Cadena = Cadena + '<p></p>' + 'Entrada';
              ...
              if ($('#MainContent_ddlTipoDocumento').val() == 7 & $('#MainContent_ddlMotivoTrabajo').val() != 14 )
            Cadena = Cadena + '<p></p>' + 'EL TIPO DOC NO ES CORRECTO';

               if ($('#MainContent_ddlTipoDocumento').val() != 7 & $('#MainContent_ddlMotivoTrabajo').val() == 14 )
            Cadena = Cadena + '<p></p>' + 'LA OPERACION NO ES CORRECTO';
...
function F_GrabarDocumento(){
    ...
    try 
        ...
        var CodDireccion =$('#MainContent_ddlDireccion').val().split('-')[0];
                ...
                if (str_resultado_operacion == "1") 
                {
                    ...
                    if (str_mensaje_operacion=='Se Grabo Correctamente')
                    {
                         ...
                         if ($('#MainContent_chkImpresion').is(':checked')) 
                                F_ImprimirDocumento(result.split('~')[2],CodMenu);
...
function F_ConfirmarRegistroPopUP(Fila) {
    ...
    if ($(lblEstado).text() === 'CONFIRMADO') {
     ...
     $('#hfConfirmacion').val($(lblCodMarcaGv).val());

      $('#div_Confirmacion').dialog({
               resizable: false,
               modal: true,
               title: "Observacion de Nota de Ingreso",
               title_html: true,
               height: 190,
               width: 470,
               autoOpen: false
           });          
        $('#div_Confirmacion').dialog('open');

    return false;
}


//function F_ConfirmarRegistro()
...
//    {
//       toastr.warning('SERIE NO VALIDA, SE CONFIRMA EN EL PUNTO DE PARTIDA');
//       return false;
//    }           
...
//    return false;
    ...
    var objParams = {
                          Filtro_Codigo: $('#hfConfirmacion').val(),
                          Filtro_Serie: '',
                          Filtro_Numero: $('#MainContent_txtNumeroConsulta').val(),
                          Filtro_Desde: $('#MainContent_txtDesde').val(),
                          Filtro_Hasta: $('#MainContent_txtHasta').val(),
                          ...
                          Filtro_CodTipoDoc: 7,
                          Filtro_CodTipoDocSust: $('#MainContent_ddlTipoDocConsulta').val() , 
                          Filtro_Observacion:$("#MainContent_txtObservacionConfirmacion").val()  
    };
...
function F_ReemplazarDocumento(Fila) {
    if (F_PermisoOpcion(CodigoMenu, CodigoInterno, 'Editar') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informacion
        try {
            ...
            var hfCodAlmacenPartida = "#"+imgID.replace("imgReemplazar","hfCodAlmacenPartida");
            var hfCodAlmacenLlegada = "#"+imgID.replace("imgReemplazar","hfCodAlmacenLlegada");
            var hfCodManual = "#"+imgID.replace("imgReemplazar","hfCodManual");
...
//            if ($(hfCodAlmacenPartida).val()!=$(hfCodAlmacenLlegada).val()) 
//            {
//                toastr.warning('ESTA NOTA INGRESO NO SE PUEDE MODIFICAR, TIENE QUE MODIFICAR LA GUIA EN EL PUNTO PARTIDA');
//                return false;
//            }
...
function F_VistaPreliminar(Fila,CodMenu){
    ...
    if (CodMenu == 117)
    {
        ...
        {
        var lblCodigo = '#' + imgID.replace('imgPdf', 'hfCodigo');    
        var lblEstado = '#' + imgID.replace('imgPdf', 'lblEstado');
    }

    if ($(lblEstado).text()=='ANULADO')
        {
            toastr.warning("LA NOTA DE INGRESO SE ENCUENTRA ANULADO");
            return false;
        }
    
    F_ImprimirDocumento($(lblCodigo).val(),CodMenu);
}

function F_ImprimirDocumento(Codigo,CodMenu) {
        var rptURL = '';
        var Params = 'width=' + (screen.width * 0.48) + ', height=' + (screen.height * 0.40) + ', top=0, left=0, directories=no, menubar=no, toolbar=no, location=no, resizable=yes, scrollbars=yes, titlebar=yes';
        var TipoArchivo = 'application/pdf';
...
if (!F_SesionRedireccionar(AppSession)) return false;
        ...
        var hfCodAlmacenPartida = "#" + imgID.replace('imgAnularDocumento','hfCodAlmacenPartida');   
        var hfCodAlmacenLlegada = "#"+imgID.replace("imgAnularDocumento","hfCodAlmacenLlegada");
        var hfCodManual = "#"+imgID.replace("imgAnularDocumento","hfCodManual");
        ...
        if($(hfCodManual).val()!='0'){
        if ($(hfCodAlmacenPartida).val()!=$(hfCodAlmacenLlegada).val()) 
        {toastr.warning('ESTA NOTA INGRESO NO SE PUEDE ANULAR, TIENE QUE MODIFICAR LA GUIA EN EL PUNTO PARTIDA');
            return false;
        } 
        }
...
function F_EliminarRegistro(Fila) {
 ...
 try 
    ...
    var hfCodManual = "#"+imgID.replace("imgEliminarDocumento","hfCodManual");

         var hfCodAlmacenPartida = "#" + imgID.replace('imgEliminarDocumento','hfCodAlmacenPartida');   
        var hfCodAlmacenLlegada = "#"+imgID.replace("imgEliminarDocumento","hfCodAlmacenLlegada");
        ...
        if($(hfCodManual).val() !="0"){
          if ($(hfCodAlmacenPartida).val()!=$(hfCodAlmacenLlegada).val()) 
        {
            toastr.warning('ESTA NOTA INGRESO NO SE PUEDE ELIMINAR, TIENE QUE ELIMINARSE LA GUIA EN EL PUNTO PARTIDA');
            return false;
        }}
...
function F_Abrir(Fila) {
    ...
    try {
     ...
     if ($(lblNumero).text().substring(0, 4)=='0250' | $(lblNumero).text().substring(0, 4)=='0211' | $(lblNumero).text().substring(0, 4)=='0173' | $(lblNumero).text().substring(0, 4)=='0270')
    {
       toastr.warning('SERIE NO VALIDA, SE ABRE EN EL PUNTO DE PARTIDA');
       return false;
    }     
...
var CodDireccion =0 ;
if ($('#MainContent_ddlDireccion').val() != null) CodDireccion =$('#MainContent_ddlDireccion').val().split('-')[0];
 
 var Cadena ='Ingresar los sgtes datos :';

//     if($('#MainContent_ddlTipoDocumento').val() != 10)
//          Cadena=Cadena + '<p></p>' + 'EL DOCUMENTO TIENE QUE SER GUIA REMISION';

     if($('#MainContent_txtSerie').val() == null | $.trim($('#MainContent_txtSerie').val()) == '')
           Cadena=Cadena + '<p></p>' + 'SERIE';

     if($('#MainContent_txtNumero').val() == null | $.trim($('#MainContent_txtNumero').val()) == '' | $('#MainContent_txtNumero').val() =='00000000')
           Cadena=Cadena + '<p></p>' + 'NUMERO';
 ...
 MostrarEspera(true);
    $.ajax({
        ...
        dataType: "json",
        data: "{'CodDireccion':'" + CodDireccion  + "','CodTipoDoc':'" + $('#MainContent_ddlTipoDocumento').val()  + "','SerieDoc':'" + $('#MainContent_txtSerie').val()  + "','NumeroDoc':'" + $('#MainContent_txtNumero').val()  + "'}",
        success: function (dbObject) {
            MostrarEspera(true);
            ...
            if(data.ltrasladosDet.length>0)
            {
                    ...
                    $.each(data.ltrasladosDet, function (index, item) {
                            var objDetalle = {
                                 ...
                                 CodDetalle: 0,
                                 Acuenta: 0,
                                 CodTipoDoc: $('#MainContent_ddlTipoDocumento').val()
                ...
                {
                    $('#hfCodigoTemporal').val(result.split('~')[3]);                    
                    $('#MainContent_txtObservacion').val(data.Observacion2);
                    $('#MainContent_txtEmision').val(data.EmisionStr);
                    $('#MainContent_ddlMotivoTrabajo').val(data.CodMotivoTraslado);
                    F_Update_Division_HTML('div_grvDetalleArticulo', result.split('~')[4]);
                    $('#lblCantidadRegistro').text(F_Numerar_Grilla("grvDetalleArticulo", "lblProducto"));
                    if (result.split('~')[2]=='Los Producto(s) se han agregado con exito')
...
function F_Inicializar_TextBox() {  
    ...
    $('#MainContent_txtObservacionConfirmacion').css('background', '#FFFFE0');
...
function F_BloquearCampos(Valor)
{
   ...
   $("#MainContent_ddlDireccion").prop('disabled', Valor);  
