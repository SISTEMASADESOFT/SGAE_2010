var AppSession = "../Inventario/GuiaRemisionUnitarioReeim.aspx";
...
$(document).ready(function () {
    ...
    if (!F_SesionRedireccionar(AppSession)) return false;
     ...
     $('#MainContent_btnTraerNotaIngreso').click(function () {
    if (!F_SesionRedireccionar(AppSession)) return false;
        try {
        ...
        try {
            if (!F_ValidarGrabarDireccion())
                ...
                break;
                 ...
                 hfCodigoTemporal='#hfCodigoTemporal';
                    FlagTraslado=1;
                    hfProveedor='#MainContent_txtTransportista';
                    hfNroRuc='#MainContent_txtNroRucTransportista';
                break;
                 case 4:
                    ddldireccion='';
            ...
            if (confirm("ESTA SEGURO DE GRABAR LA DIRECCION"))
                ...
                '#MainContent_txtEmailMultiple2',hfCodCtacte,ddldireccion,'#hfCodDireccionTemporal','#MainContent_txtDistritoMultiple',ddldireccionDestino,FlagTraslado,hfNroRuc,hfProveedor);
    ...
    $(document).on("change", "select[id $= 'MainContent_ddlSerie']", function () {
        F_Mostrar_Correlativo($('#MainContent_ddlTipoDocumento').val());
           ...
           BloqueoTipoTransportista($(Cuerpo + 'ddlSerie option:selected').text(),(Cuerpo + 'txtNumeroGuia'),(Cuerpo + 'txtFechaTraslado')
                 ,(Cuerpo + 'ddldireccionNuevaDestino'),('#hfProveedor'),('#hfNroRuc')               
...
function F_CamposAlternativas(Campos) {
    ...
    switch (Campos) {
        ...
        case "MainContent_txtVencimiento":
            ...
            break;
             case "MainContent_txtNotaingreso":
            if ($("#MainContent_txtNotaingreso").val().trim() != "") {
                if (isNaN($("#MainContent_txtNotaingreso").val().trim()))
                    $("#MainContent_txtNotaingreso").val('');
                else
                    $('#MainContent_btnTraerNotaIngreso').click();
            }
...
function F_DestinoGuiaInterna() {
     var CodTipoDoc = $("#MainContent_ddlTipoDocumento").val();       
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: '../Servicios/Servicios.asmx/F_DestinoGuiaInterna',
        //data: "{'CodTipoDoc':'" + CodTipoDoc+"'}", //agregar parametro ms de FlagUsuariosInactivos
        ...
        success: function (dbObject) {
            ...
            try {
                 $('#MainContent_ddldireccionNuevaDestino').empty();
                  ...
                  $("#MainContent_txtProveedor").val(data.rows2[0].Proveedor);
                    $("#hfNroRuc").val(data.rows2[0].NroRuc);
                    $('#hfCodCtaCte').val(data.rows2[0].CodCtaCte);
            ...
            (
                ...
                function (result) {
                    ...
                    if (str_resultado_operacion == "1") {
                        ...
                        $('#MainContent_txtNotaingreso').css('background', '#FFFFE0');
...
function F_GrabarDocumento(){
    ...
    try 
        ...
        var objParams = {
                ...
                Filtro_CodNotaingreso: $('#hfcodnotaingreso').val()
            };
        ...
        try {
            ...
            ,'#MainContent_txtProveedor',0,Ubigeo,Direccion,'#MainContent_ddldireccionNueva','#MainContent_ddldireccionNuevaDestino',1);
                 ...
                 ,(Cuerpo + 'txtPlacaTraslado') ,(Cuerpo + 'txtLicenciaGuia'));
                    ...
                    BuscarDireccionCotizacion('#hfCodigoTemporal', $('#hfCoddireccionDestino').val(),'#MainContent_ddldireccionNuevaDestino','');
                }
                else {
...
//                         F_FormatosImpresion2();
                        if (Div == 'div_serie')
                            ...
                            if(($("#hfcodnotaingreso").val()>0))
                        BuscarDireccionCotizacion('#hfCodigoTemporal', $('#hfCoddireccionDestino').val(),'#MainContent_ddldireccionNuevaDestino','',CodDoc);
                    }
...
// }; 
  if (APITransportista == "Usuario No Encontrado") {
        ...
        $.ajax({
            ...
            ,'#MainContent_txtTransportista',1,Ubigeo,direccion.replace(distrito, ""),'#MainContent_ddldireccionNuevaTransportista','',1);
                }
                catch (x) { }
                MostrarEspera(false);
            },
...
function F_Pedidos() {
    ...
    $.ajax({
        ...
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        url: 'GuiaRemisionUnitarioReeim.aspx/F_Consulta_Notaingreso_Pendientes_NET',
        dataType: "json",
        data: "{'CodAlmacen':'" + $('#hfCodAlmacen').val()  + "'}",
        success: function (dbObject) {
            MostrarEspera(true);
            ...
            try {
                ...
                $.each(data.rows, function (index, item) {
                    var fmt2 =   ' <li style="width: 220px; height:30px; padding-top:0px; display:block"> <div class="linea-button"> ' +
                                ...
                                '             <input type="hidden" id="@ID_CODALMACENLLEGADA" value="@CodAlmacenLlegada"> ' +
                                '             <input type="hidden" id="@ID_ALMACEN" value="@Almacen"> ' +
                                '       </td> ' +
								'  		<td style="width: 100;padding-left:45px"> ' +
                                '             <label style="font-weight: bold; font-size: 18px;">@NroDocumento</label> ' +
                                '       </td> ' +
								'  		<td style="padding-left:5px"> ' +
                                '             <label style="font-weight: bold; font-size: 18px;">@Almacen</label> ' +
                                '       </td> ' +
								'  		<td style="width: 25"> ' +
                    ...
                    var li = fmt2.replace('@Nro', Count).replace('@ID', 'NOTAINGRESO_' + Count).replace('@CodMovimiento', item.CodMovimiento)
                                        ...
                                        .replace('@ID_CODALMACENLLEGADA', 'NOTAINGRESO_CODALMACENLLEGADA_' + Count).replace('@CodAlmacenLlegada', item.CodAlmacenLlegada)
                                        .replace('@ID_ALMACEN', 'NOTAINGRESO_ALMACEN_' + Count).replace('@Almacen', item.Almacen)
                                        .replace('@Almacen', item.Almacen)
...
function F_AgregarTemporalCTxBoton(Boton) {
    $('#MainContent_txtNotaingreso').val($('#NOTAINGRESO_NUMERO_' + Boton.split('_')[2]).val());
    $('#MainContent_ddlDestino').val($('#NOTAINGRESO_CODALMACENLLEGADA_' + Boton.split('_')[2]).val());
    F_AgregarTemporalCTxNumero();
}
...
function F_AgregarTemporalCTxNumero() {
    ...
    $.ajax({
        ...
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        url: 'GuiaRemisionUnitarioReeim.aspx/F_Obtenernotaingreso_PorNumero_Net',
        dataType: "json",
        data: "{'NumeroNotaingreso':'" + $(Contenedor + 'txtNotaingreso').val()  + "'}",
        success: function (dbObject) {
            MostrarEspera(true);
            var data = dbObject.d;
            try {
        ...
        var tasaigv = parseFloat($("#MainContent_ddlIgv option:selected").text()) + parseFloat(1);
                    ...
                    //Array de Items
                    $.each(data.lnotaingresosalidaDet, function (index, item) {
                ...
                if (str_resultado_operacion == "1") 
                {
                    ...
                    $('#MainContent_txtSubTotal').val(result.split('~')[7]);
                    $('#MainContent_txtDsctoTotal').val(result.split('~')[8]);
                    $('#MainContent_txtDsctoTotal').val(data.Observacion);
                    F_Update_Division_HTML('div_grvDetalleArticulo', result.split('~')[4]);
                    $('#MainContent_lblNumRegistros').text(F_Numerar_Grilla("grvDetalleArticulo", 'lblProducto')); 
                    if (result.split('~')[2]=='Los Producto(s) se han agregado con exito')
                        toastr.warning('Los Producto(s) se han agregado con exito');
                    ...
                    $('#MainContent_txtNotaingreso').val('');
                    $("#hfMenorPrecioAgregar").val(0);
                    $("#hfMenorPrecioAgregar").val(data.CodAlmacenDestino);
                    $("#MainContent_txtProveedor").val(data.Cliente);
                    //$("#MainContent_txtObservacionGuiaEdicion").val(data.Observacion);
                    $("#hfNroRuc").val(data.NroRuc);
                    $("#hfcodnotaingreso").val(data.CodMovimiento);
                    $('#hfCodCtaCte').val(data.CodCtaCte);
                    $('#hfCoddireccionDestino').val(data.CodDireccionDestino);
                    $("#MainContent_ddlTipoDocumento").val(4);
                    $('.ccsestilo').css('background', '#FFFFE0');     
                    //F_LimpiarGrillaConsulta();
                    $('#MainContent_txtArticulo').focus();
