var AppSession = "../Inventario/NotaIngresoSalidaReeim.aspx";
            ...
            (
                ...
                function (result) {
                    ...
                    if (str_resultado_operacion == "1") 
                    {
                        ...
                        F_Update_Division_HTML('div_Almacen', result.split('~')[10]);
        ...
        if ($('#hfRemoto').val() === "0") {
            ...
            ConexionNombre      = "";  
                ...
                var objParams = {
                                    ...
                                    Filtro_CodAlmacenOrigen: $('#MainContent_ddlAlmacen').val(),                                                         
...
function F_DesconfirmarRegistro(Fila) {
//    if (F_PermisoOpcion(CodigoMenu, CodigoInterno, 'Eliminar') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informacion
 try 
        {
    var imgID = Fila.id;
    var lblCodMarcaGv = '#' + imgID.replace('imgDesConfirmar', 'hfCodigo');
    var lblEstado = '#' + imgID.replace('imgDesConfirmar', 'lblEstado');
    var lblNumero = '#' + imgID.replace('imgDesConfirmar', 'lblNumero');
    var lblCliente = '#' + imgID.replace('imgDesConfirmar', 'lblCliente');
    var hfTipoDocumento = '#' + imgID.replace('imgDesConfirmar', 'hfTipoDocumento');    
    if ($(lblEstado).text()=='ANULADO')
    {
        toastr.warning("LA NOTA SE ENCUENTRA ANULADA");
        return false;
    ...
    if ($(lblEstado).text()!='CONFIRMADO')
    {
        toastr.warning("LA NOTA NO SE ENCUENTRA CONFIRMADA");
        return false;
    }
    if(!confirm("ESTA SEGURO DE DESCONFIRMAR LA NOTA DE INGRESO "  + " : " + $(lblNumero).text() + "\nDE : " +  $(lblCliente).text()))
    return false;
    ...
    var arg = Sys.Serialization.JavaScriptSerializer.serialize(objParams);
     MostrarEspera(true);
    F_DesConfirmarRegistro_Net(arg, function (result) {
...
function F_ReemplazarDocumento(Fila) {
    if (F_PermisoOpcion(CodigoMenu, CodigoInterno, 'Editar') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informacion
        try {
            ...
            if (($(hfCodTipoDoc).val()==10 || $(hfCodTipoDoc).val()==4) & ($(lblEstado).text() == 'FACTURADO' || $(lblEstado).text() == 'ANULADO'))
            {
...
/*function F_AnularPopUP(Fila) {
    ...
    var imgID = Fila.id;
    var lblCodMarcaGv = '#' + imgID.replace('imgAnularDocumento', 'hfCodigo');
    ...
    if($(hfCodProforma).val()!=''){
        toastr.warning("ESTA NOTA DE INGRESO SOLO PUEDE SER ANULADA POR EL VENDEDOR YA QUE ESTA ASOCIADA A UNA COTIZACION");
        return false;
    }     

    if($(hfAutomatico).val()==1){
        toastr.warning('LA NOTA ES AUTOMATICA, POR LO QUE NO SE PUEDE ANULAR');
        return false;
    }
    if(!confirm("ESTA SEGURO DE ANULAR LA NOTA DE INGRESO "  + " : " + $(lblNumero).text() + "\nDE : " +  $(lblCliente).text()))
    return false;

    $('#lblCodMarcaGv').val($(lblCodMarcaGv).val());
    $('#lblEstado').val($(lblEstado).text());
    $('#lblnumero').val($(lblnumero).text());
    $('#MainContent_txtObservacionAnulacion').val('');
    $('#hflblCliente').val($(lblCliente).val());
    $('#hfTipoDocumento').val($(hfTipoDocumento).val());
    $('#hfAutomatico').val($(hfAutomatico).val());
            ...
            (
                ...
                function (result) {
                    ...
                    if (str_resultado_operacion == "1") {
                         ...
                         $('#MainContent_txtSerie').val(result.split('~')[2]);
