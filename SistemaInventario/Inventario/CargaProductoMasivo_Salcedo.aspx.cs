...
{
    public partial class CargaProductoMasivo_Salcedo : System.Web.UI.Page
    {
            ...
            if (FileUpload1.HasFile)
            {
                    ...
                    using (var package = new ExcelPackage(fileInfo))
                    {
                        ...
                        using (SqlConnection connection = new SqlConnection(sqlConnectionString))
                        {
                            ...
                            for (int row = worksheet.Dimension.Start.Row + 1; row <= worksheet.Dimension.End.Row; row++)
                            {
                                ...
                                string DETALLES = worksheet.Cells[row, 3].Text;
                                string MEDIDAA = worksheet.Cells[row, 4].Text;
                                string MEDIDAB = worksheet.Cells[row, 5].Text;
                                string MEDIDAC = worksheet.Cells[row, 6].Text;
                                string MEDIDAD = worksheet.Cells[row, 7].Text;

                                // Realizar la inserción en la base de datos
                                string insertQuery = "INSERT INTO CargaMasivaProducto (CODIGO, DESCRIPCION, DETALLES, MEDIDA A, MEDIDA B, MEDIDA C, MEDIDA D,IDCargaMasivaExcelCab)" +
                                           "VALUES (@CODIGO,@DESCRIPCION,@DETALLES,@MEDIDAA,@MEDIDAB,@MEDIDAC,@MEDIDAD,@CodCargaMasivaCab)";
                                ...
                                using (SqlCommand cmd = new SqlCommand(insertQuery, connection))
                                {
                                    ...
                                    cmd.Parameters.AddWithValue("@MEDIDAA", MEDIDAA);
                                    cmd.Parameters.AddWithValue("@MEDIDAB", MEDIDAB);
                                    cmd.Parameters.AddWithValue("@MEDIDAC", MEDIDAC);
                                    cmd.Parameters.AddWithValue("@MEDIDAD", MEDIDAD);
                ...
                catch (Exception ex)
                {
                    MensajeError = "Error en la lectura del excel. <p></p> " +
                                    ...
                                    " <b>Hoja: Inventario</b>" +
                                    " Columnas: <p></p>" +
                                    " <b>[A]=Codigo, [B]=DESCRIPCION, [C]=DETALLES, [C]=MEDIDA A, [C]=MEDIDA B, [C]=MEDIDA C, [C]=MEDIDA D</b>";
        ...
        private void ProcedimientoTrasladar(ref string Mensaje, ref int CodMovimiento)
        {
            ...
            objOperacion.F_RegistroAjustes_Excel_GRABAR_SALCEDO(objEntidad);
