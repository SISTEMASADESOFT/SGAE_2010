var AppSession = "../Inventario/GuiaRemisionUnitarioKuyay.aspx";
...
//    $('#divTabs').tabs();  
    ...
    $('#MainContent_txtProveedor').autocomplete({
        ...
        select: function (e, i) {
            ...
            $('#hfProveedor').val(i.item.Razonsocial);
            $('#hfNroRuc').val(i.item.NroRuc);
            $('#MainContent_txtNroRuc').val(i.item.NroRuc);
            $('#MainContent_txtProveedor').val(i.item.Razonsocial);
...
function F_AgregarArticulo(ControlID, DirectoBoton) {
    ...
    lblCodigoProducto_grilla = imgAgregar_grilla.replace('imgAgregar', 'hfCodigoInterno');
    ...
    $('#MainContent_txtCodigoProductoAgregar').val($(lblCodigoProducto_grilla).val());
                ...
                function (result) {
                    ...
                    if (str_resultado_operacion == "1") {
                       ...
                       $('#hfDireccion').val('');
                        ...
                        $('#MainContent_ddlTipoDocConsulta').val(10)
           
                        $('#MainContent_ddlMotivoTrabajo').val('1');   
        ...
        var tasaigv;
          ...
          if ($('#MainContent_chKConIgv').is(':checked'))
                 ...
                 tasaigv=1;
                        ...
                        CodDetalle: 0,
                        Fecha : $('#MainContent_txtFechaCaducidad').val(),
...
function F_ValidarGrabarDocumento(){
    try 
        ...
        if ($('#MainContent_ddlMotivoTrabajo').val() == null)
            ...
            Cadena = Cadena + '<p></p>' + 'MOTIVO NO VALIDO';

        if ($(Cuerpo + 'txtPartida').val() == '')
            Cadena = Cadena + '<p></p>' + 'Partida';

                 if(($(Cuerpo + 'ddlSerie option:selected').text()).substr(0, 1)=='T' ){
                 
                  var CadenaValGuia=Validacionguia($(Cuerpo + 'ddlSerie option:selected').text(),$(Cuerpo + 'txtNumeroGuia').val()
...
function F_GrabarDocumento()
{
    ...
    try 
        ...
        var FlagMotorizado=0; if ($(Contenedor + 'chkMotorizado').is(':checked'))   FlagMotorizado=1;
...
//        switch($(Contenedor + 'ddlMotivoTrabajo').val()) {
//            case "7":
//              CodTipoOperacion = 11;
//              break;
//            case "9":
...
//            break;
//            default:
//              CodTipoOperacion = 99;// estaba con 100
             switch($(Contenedor + 'ddlMotivoTrabajo').val()) {
            case "1":
              CodTipoOperacion = 1;
              break;
        ...
        var objParams = {
                ...
                if (str_resultado_operacion == "1") 
                {
                    if (str_mensaje_operacion == 'Se grabo correctamente')
                    {
                         ...
                         F_ImprimirGuia(result.split('~')[2],CodMenu);
                         F_Nuevo();
                    }
                    else
                    {
...
function F_Nuevo(){
       ...
       $('#MainContent_txtObservacionGuiaEdicion').val('');        
...
function F_ImprimirGuia(Codigo, CodMenu) {
...
function F_ReemplazarDocumento(Fila) {
    if (F_PermisoOpcion(CodigoMenu, CodigoInterno, 'Editar') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informacion
        try {
            ...
            if(($(lblNumero).text()).substr(0, 1)=='T'){
            ...
            F_ReemplazarFactura_NET(arg, function (result) {
                ...
                if (str_resultado_operacion == "1") {
                    ...
                    $('#MainContent_ddlDestino').val(result.split('~')[19]);
                    ...
                    BuscarDireccionCotizacion('#hfCodigoTemporal',result.split('~')[19],'#MainContent_ddldireccionNuevaDestino','',Codtipodoc);                   
                }
                else {
...
//        }
        ...
        var codProducto = $('#hfCodProductoAgregar').val();
        var descripcion = $('#MainContent_txtArticuloAgregar').val();
        var loteNuevo = $('#MainContent_txtLote').val().trim();
        ...
        $('#MainContent_grvDetalleArticulo .chkDelete :checkbox').each(function () {
            ...
            var hfcodarticulodetalle_grilla = chkDel.replace('chkEliminar', 'hfCodArticulo');
            var codProdExistente = $(hfcodarticulodetalle_grilla).val();
 
            var txtlote_grilla = chkDel.replace('chkEliminar', 'txtLote');
            var loteExistente = $(txtlote_grilla).val();

             
            if (codProducto === codProdExistente && loteNuevo === loteExistente) {
                toastr.warning("El producto " + descripcion + " ya tiene registrado el lote: " + loteNuevo);
                duplicado = true;
                return false;  
            }
        });
...
//     
//        if ($("#MainContent_txtArticuloAgregar").val().trim() === '')
//            cadena += "CAMPO DESCRIPCIÓN ARTÍCULO <p></p>";

//        if ($("#MainContent_txtCantidad").val().trim() === '') {
//            cadena += "CAMPO CANTIDAD <p></p>";
//        } else if (isNaN($("#MainContent_txtCantidad").val())) {
//            $("#MainContent_txtCantidad").val(1).focus().select();
//            cadena += "CAMPO CANTIDAD <p></p>";
//        }

//        if ($("#MainContent_txtPrecioDisplay").val().trim() === '') {
//            cadena += "CAMPO PRECIO ARTÍCULO <p></p>";
//        } else if (isNaN($("#MainContent_txtPrecioDisplay").val())) {
//            $("#MainContent_txtPrecioDisplay").val('0.00').focus().select();
//            cadena += "CAMPO PRECIO <p></p>";
//        }

//        if ($("#MainContent_txtLote").val().trim() === '')
//            cadena += "CAMPO LOTE <p></p>";

//        var fechaCaducidad = $("#MainContent_txtFechaCaducidad").val().trim();
//        if (fechaCaducidad === '') {
//            cadena += "CAMPO FECHA CADUCIDAD <p></p>";
//        } else {
//            var partes = fechaCaducidad.split('/');
//            var dia = parseInt(partes[0], 10);
//            var mes = parseInt(partes[1], 10) - 1;
...
//            if (fechaIngresada < hoy) {
//                cadena += "La fecha de vencimiento debe ser mayor a la actual <p></p>";
//            }
//        }
...
//        if (P_VALIDASTOCK == "1") {
//            if ((Number($("#MainContent_txtStockAgregar").val()) - Number($("#MainContent_txtCantidad").val())) 
//                < Number(P_VALIDASTOCK_MONTO_MINIMO)) {
//                toastr.warning("STOCK INSUFICIENTE");
//                return false;
//            }
//        }
...
//            var codProdExistente = $(hfcodarticulodetalle_grilla).val();
//            var loteExistente = $(hflote_grilla).val();

//            // mismo producto
//            if (codProducto === codProdExistente) {
//                // mismo lote
//                if (loteNuevo === loteExistente) {
//                    toastr.warning("El producto ya existe con el mismo lote: " + descripcion + " - Lote " + loteNuevo);
//                    duplicado = true;
//                    return false; // corta el each
//                } else {
//                    toastr.warning("El producto ya se encuentra agregado: " + descripcion);
//                    duplicado = true;
//                    return false;
//                }
//            }
...
function F_ActualizarVencimiento(Fila) {
    try {
        ...
        var txtDescripcion = txtFechaVencimiento.replace('txtFechaVencimiento', 'lblProducto');
        ...
        var objParams = {
            ...
            Filtro_CodDetDocumentoVenta: $(hfCodDetalle).val(),
            Filtro_CodMoneda: $('#MainContent_ddlMoneda').val(),
            Filtro_Descripcion: $(txtDescripcion).text(),
           // Filtro_TasaIgv: tasaigv,
            Filtro_FechaVencimiento: $(txtFechaVencimiento).val(), 
            Filtro_Lote: $(txtLote).val(),
            Filtro_NotaPedido: 0,
        ...
        F_ActualizarGuia_Net(arg, function (result) {
            var str_resultado_operacion = "";
            var str_mensaje_operacion = "";
...
function F_ActualizarLote(Fila){
 try{
    ...
    var txtDescripcion = txtLote.replace('txtLote', 'lblProducto');
...
function F_ConfirmarRegistro(Fila) {
 ...
 try 
    ...
    var hfCodTipoDoc = '#' + imgID.replace('imgConfirmar', 'hfCodTipoDoc');    
 
    if ($(lblEstado).text() == 'FACTURADO' || $(lblEstado).text() == 'ANULADO'  || $(lblEstado).text() == 'CONFIRMADO')
        {toastr.warning("LA GUIA SE ENCUENTRA "+$(lblEstado).text());
        return false;}

//    if ($(hfCodTipoDoc).val()==10)
//    {
//       toastr.warning('LAS GUIAS REMISION SE CONFIRMAN EN EL DESTINO');
//       return false;
//    }   
    ...
    var hfAutomatico = '#' + imgID.replace('imgEliminarDocumento', 'hfAutomatico');
     ...
     if ($(lblEstado).text() == 'FACTURADO' | $(lblEstado).text() == 'CONFIRMADO')
        ...
        return false;}
         ...
         if ($(lblNumero).text().substring(0, 4)=='0250' | $(lblNumero).text().substring(0, 4)=='0211' | $(lblNumero).text().substring(0, 4)=='0173' | $(lblNumero).text().substring(0, 4)=='0270')
    {
       toastr.warning('SERIE NO VALIDA, NO SE PUEDE ELIMINAR');
       return false;
    }   

         if(($(lblNumero).text()).substr(0, 1)=='T'){
                toastr.warning('LA GUIA ES ELECTRONICA, POR LO QUE NO SE PUEDE ELIMINAR');
                return false;
            }
...
function F_Abrir(Fila) {
    ...
    try {
        ...
        var hfCodTipoDoc = '#' + imgID.replace('imgAbrir', 'hfCodTipoDoc');   

        if ($(hfCodTipoDoc).val()==10)
        {
           toastr.warning('LAS GUIAS REMISION SE ABREN EN EL DESTINO');
           return false;
        }
        ...
        try {
            ...
            var objParams = {
              ...
              if(($(lblNumero).text()).substr(0, 1)=='T'){
                toastr.warning('LA GUIA ES ELECTRONICA, POR LO QUE NO SE PUEDE ABRIR');
                return false;
            }
...
//F_LimpiarHiddenTrasportista();
    ...
    if ($('#MainContent_txtNroRucTransportista').val() != "1" & Index ==-1 ) {
       if (isNaN($('#MainContent_txtNroRucTransportista').val()) )
        {
            $('#MainContent_txtNroRucTransportista').val('');
            $('#MainContent_txtNroRucTransportista').focus();
...
function F_BuscarNotaIngreso(Fila) {
   var Cadena ='Ingresar los sgtes datos :';
    ...
    if($('#MainContent_ddlTipoDocumento').val() != 4)
          Cadena=Cadena + '<p></p>' + 'EL DOCUMENTO TIENE QUE SE GUIA INTERNA';

    if($('#MainContent_ddlSerie').val() == null)
          Cadena=Cadena + '<p></p>' + 'SERIE';

    if($('#MainContent_txtNumero').val() == null)
          Cadena=Cadena + '<p></p>' + 'NUMERO';

    if($.trim($('#MainContent_txtProveedor').val()) == '' | $('#hfCodCtaCte').val()==0)
          Cadena=Cadena + '<p></p>' + 'DESTINATARIO';
 ...
 MostrarEspera(true);
    $.ajax({
        ...
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        url: 'GuiaRemisionUnitarioAlvarado.aspx/F_ObtenerNotaIngreso_PorID_Net',
        dataType: "json",
        data: "{'CodDocumentoVentaDireccion':'" + $('#MainContent_ddldireccionNuevaDestino').val()  + "','CodTipoDocSust':'" + $('#MainContent_ddlTipoDocumento').val()  + "','SerieDocSust':'" + $('#MainContent_ddlSerie option:selected').text()  + "','NumeroDocSust':'" + $('#MainContent_txtNumero').val()  + "'}",
        success: function (dbObject) {
            MostrarEspera(true);
            ...
            if(data.lNotaingresoDet.length>0)
            {
                ...
                {
                       $('#hfCodigoTemporal').val(result.split('~')[3]);                      
                       $('#MainContent_txtObservacionGuiaEdicion').val(data.Observacion2);
                       $('#MainContent_txtEmision').val(data.EmisionStr);
                       F_Update_Division_HTML('div_grvDetalleArticulo', result.split('~')[4]);  
                       
                       if (data.NroRucPartida == NroRuc)
        ...
        toastr.warning("ERROR DETECTADO: " + xhr.responseJSON.Message)
          $('#MainContent_txtObservacionGuiaEdicion').val(xhr.responseJSON.Message);
          MostrarEspera(false);
        }
    });
 return false
...
function F_BuscarFacturaCompra(Fila) {
 ...
 MostrarEspera(true);
    $.ajax({
        ...
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        url: 'GuiaRemisionUnitarioAlvarado.aspx/F_ObtenerFacturaCompra_PorID_Net',
        dataType: "json",
...
function F_BloquearCampos(Valor)
{
   $("#MainContent_ddlTipoDocumento").prop('disabled', Valor);
   $("#MainContent_txtEmision").prop('disabled', Valor);
   $("#MainContent_ddlSerie").prop('disabled', Valor);
   $("#MainContent_txtNumero").prop('disabled', Valor);
   $("#MainContent_ddlTipoTransportista").prop('disabled', Valor);
   $("#MainContent_txtProveedor").prop('disabled', Valor);
   $("#MainContent_ddlMotivoTrabajo").prop('disabled', Valor);
   $("#MainContent_ddldireccionNuevaDestino").prop('disabled', Valor);
   $("#MainContent_ImageButton1").prop('disabled', Valor);
   $("#MainContent_txtNroRucTransportista").prop('disabled', Valor);
   $("#MainContent_txtTransportista").prop('disabled', Valor);
   $("#MainContent_ddldireccionNuevaTransportista").prop('disabled', Valor);
   $("#MainContent_ImageButton2").prop('disabled', Valor);
   $("#MainContent_txtNuBultos").prop('disabled', Valor);
   $("#MainContent_txtPeso").prop('disabled', Valor);
   $("#MainContent_ddlcodunidadpeso").prop('disabled', Valor);
   $("#MainContent_txtConductorDNI").prop('disabled', Valor);
   $("#MainContent_txtConductorRazonSocial").prop('disabled', Valor);
   $("#MainContent_txtPlacaTraslado").prop('disabled', Valor);
   $("#MainContent_txtMarcaGuia").prop('disabled', Valor);
   $("#MainContent_txtLicenciaGuia").prop('disabled', Valor);
   $("#MainContent_btnBuscar").prop('disabled', Valor);
   $("#MainContent_btnAgregarProducto").prop('disabled', Valor);
   $("#MainContent_btnEliminar").prop('disabled', Valor);
   $('#MainContent_grvDetalleArticulo .chkDelete :checkbox').each(function () {
         chkDel = '#' + this.id;
         txtCantidad = chkDel.replace('chkEliminar', 'txtCantidad');
         $(txtCantidad).prop('disabled', Valor);
   });
   return false;
}
...
function F_ImprimirFacturaGrilla(Fila,CodMenu) {
        var imgID = Fila.id;
        var Codigo = $('#' + imgID.replace('imgImprimirGuia', 'hfCodigo')).val() ; 
        var Estado = $('#' + imgID.replace('imgImprimirGuia', 'lblEstado')).val() ;    
