var AppSession = "../Inventario/KardexKarinaFisico.aspx";
...
var C = 0;
    ...
    $('#MainContent_' + NombreGrilla + ' .detallesart3').each(function () {
        ...
        if (C == 0)
            SaldoInicial = parseFloat($('#MainContent_' + NombreGrilla + '_' + NombreLabelInicial + '_' + C).text());                              
        C++;
    });

    $('#MainContent_lblIngresos').text(parseFloat(Ingreso) + parseFloat(SaldoInicial));
    $('#MainContent_lblSalidas').text(Salida);
    $('#MainContent_lblStock').text(parseFloat(SaldoInicial) + parseFloat(Ingreso) - parseFloat(Salida));
    return false;
}
var auImportacion;
