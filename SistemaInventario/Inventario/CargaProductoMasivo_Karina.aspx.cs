...
{
    public partial class CargaProductoMasivo_Karina : System.Web.UI.Page
    {
                    ...
                    using (var package = new ExcelPackage(fileInfo))
                    {
                        ...
                        using (SqlConnection connection = new SqlConnection(sqlConnectionString))
                        {
                            ...
                            for (int row = worksheet.Dimension.Start.Row + 1; row <= worksheet.Dimension.End.Row; row++)
                            {
                                ...
                                //string PRECIO = worksheet.Cells[row, 13].Text;
                                //string PRECIO2 = worksheet.Cells[row, 14].Text;
                                //string PRECIO3 = worksheet.Cells[row, 15].Text;
                                //string CODMONEDACOSTO = worksheet.Cells[row, 16].Text;
                                //string COSTO = worksheet.Cells[row, 17].Text;
                                //string UM = worksheet.Cells[row, 18].Text;
                                //string STOCK = worksheet.Cells[row, 19].Text;
                                ...
                                string CODIGODETRACCION = worksheet.Cells[row, 10].Text;
                                string TIPOPRODUCTO = worksheet.Cells[row, 11].Text;
                                                           
//                                string insertQuery = @"INSERT INTO CargaMasivaProducto (
//                            CODIGOUNICO, FAMILIA, CODIGO, CODIGO2, MEDIDA, POSICION, MODELO, AÑO, DESCRIPCION,
//                            MARCA, UBICACION, CODMONEDAPRECIO, PRECIO, PRECIO2, PRECIO3,
//                            CODMONEDACOSTO, COSTO, UM, STOCK, IDCargaMasivaExcelCab)
//                            VALUES (
//                            @CODIGOUNICO, @FAMILIA, @CODIGO, @CODIGO2, @MEDIDA, @POSICION, @MODELO, @AÑO, @DESCRIPCION,
//                            @MARCA, @UBICACION, @CODMONEDAPRECIO, @PRECIO, @PRECIO2, @PRECIO3,
//                            @CODMONEDACOSTO, @COSTO, @UM, @STOCK, @CodCargaMasivaCab)";

                                string insertQuery = @"INSERT CargaMasivaProducto(CODIGOUNICO, 
                                 Descripcion,Marca,STOCK,Costo,CODMONEDACOSTO,PRECIO,CODMONEDAPRECIO,Familia,CodDetraccion,CodCategoriaIgv,IDCargaMasivaExcelCab)
  
                 VALUES (@CODIGOUNICO,@Descripcion,@Marca,@STOCK,@Costo,@CODMONEDACOSTO,@PRECIO,@CODMONEDAPRECIO,@Familia,@CODIGODETRACCION,@TIPOPRODUCTO,@CodCargaMasivaCab)";
                                ...
                                using (SqlCommand cmd = new SqlCommand(insertQuery, connection))
                                {
                                    ...
                                    cmd.Parameters.AddWithValue("@CODIGODETRACCION", CODIGODETRACCION);
