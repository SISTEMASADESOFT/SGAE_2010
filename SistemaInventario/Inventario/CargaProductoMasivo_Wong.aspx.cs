...
{
    public partial class CargaProductoMasivo_Wong : System.Web.UI.Page
    {
                    ...
                    using (var package = new ExcelPackage(fileInfo))
                    {
                        ...
                        using (SqlConnection connection = new SqlConnection(sqlConnectionString))
                        {
                            ...
                            for (int row = worksheet.Dimension.Start.Row + 1; row <= worksheet.Dimension.End.Row; row++)
                            {
                                ...
                                string CATEGORIA = worksheet.Cells[row, 7].Text;
                                string TIPOPRODUCTO = worksheet.Cells[row, 8].Text;
                                string PRECIO = worksheet.Cells[row, 9].Text;
                                string PRECIO2 = worksheet.Cells[row, 10].Text;
                                string PRECIO3 = worksheet.Cells[row, 11].Text;
                                string MONEDAPRECIO = worksheet.Cells[row, 12].Text;
                                string COSTO = worksheet.Cells[row, 13].Text;
                                string MONEDACOSTO = worksheet.Cells[row, 14].Text;
                                string STOCK = worksheet.Cells[row, 15].Text;
                                string MEDIDA = worksheet.Cells[row, 16].Text;                              
                                string CODIGO = worksheet.Cells[row, 17].Text;


                                // Realizar la inserción en la base de datos
                                string insertQuery = "INSERT INTO CargaMasivaProducto (CODIGOUNICO,DESCRIPCION,MARCA,OBSERVACION,UBICACION,FAMILIA,CATEGORIA,TIPOPRODUCTO,PRECIO,PRECIO2,PRECIO3,CODMONEDAPRECIO,COSTO,CODMONEDACOSTO,STOCK,MEDIDA,CODIGO,IDCargaMasivaExcelCab)" +
                                           "VALUES (@CODIGOUNICO,@DESCRIPCION,@MARCA,@OBSERVACION,@UBICACION,@FAMILIA,@CATEGORIA,@TIPOPRODUCTO,@PRECIO,@PRECIO2,@PRECIO3,@MONEDAPRECIO,@COSTO,@MONEDACOSTO,@STOCK,@MEDIDA,@CODIGO,@CodCargaMasivaCab)";
