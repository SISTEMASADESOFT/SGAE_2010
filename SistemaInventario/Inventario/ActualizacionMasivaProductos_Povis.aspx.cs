...
{
    public partial class ActualizacionMasivaProductos_Povis : System.Web.UI.Page
    {
        ...
        protected void btnImport_Click(object sender, EventArgs e)
        {
            ...
            objOperacion.F_LGPRODUCTOS_ACTUALIZACION_MASIVA_POVIS(objEntidad);
                    ...
                    using (var package = new ExcelPackage(fileInfo))
                    {
                        ...
                        using (SqlConnection connection = new SqlConnection(sqlConnectionString))
                        {
                            ...
                            for (int row = worksheet.Dimension.Start.Row + 1; row <= worksheet.Dimension.End.Row; row++)
                            {
                                ...
                                string DESCRIPCION = worksheet.Cells[row, 5].Text;
                                string MARCA = worksheet.Cells[row, 6].Text;
                                string UBICACION = worksheet.Cells[row, 7].Text;
                                string ORDEN = worksheet.Cells[row, 8].Text;
                                string STOCKMINIMO = worksheet.Cells[row, 9].Text;
                                string STOCKMAXIMO = worksheet.Cells[row, 10].Text;
                                string MEDIDAS = worksheet.Cells[row, 11].Text;
                                string CATEGORIA = worksheet.Cells[row, 12].Text;
                                string ESPESOR = worksheet.Cells[row, 13].Text;
                                string PESO = worksheet.Cells[row, 14].Text;
                             
                                // Realizar la inserción en la base de datos
                                string insertQuery = "INSERT INTO CargaMasivaProducto (CODIGOINTERNO,FAMILIA,CODIGO,CODIGO2,DESCRIPCION,MARCA,UBICACION,ORDEN,STOCKMINIMO,STOCKMAXIMO,MEDIDAS,CATEGORIA,ESPESOR,PESO,IDCargaMasivaExcelCab)" +
                                           "VALUES (@CODIGOINTERNO,@FAMILIA,@CODIGO,@CODIGO2,@DESCRIPCION,@MARCA,@UBICACION,@ORDEN,@STOCKMINIMO,@STOCKMAXIMO,@MEDIDAS,@CATEGORIA,@ESPESOR,@PESO,@IDCargaMasivaExcelCab)";
                                ...
                                using (SqlCommand cmd = new SqlCommand(insertQuery, connection))
                                {
                                    ...
                                    cmd.Parameters.AddWithValue("@IDCargaMasivaExcelCab", HiddenField1.Value);
