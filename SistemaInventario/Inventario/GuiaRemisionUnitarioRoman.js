var AppSession = "../Inventario/GuiaRemisionUnitarioRoman.aspx";
...
$(document).ready(function () {
    ...
    $('#MainContent_txtProveedor').autocomplete({
        ...
        select: function (e, i) {
           ...
           F_GuardarDireccion($('#hfNroRuc').val(),'#hfCodigoTemporal',$('#hfCodCtaCte').val()
            ,'#MainContent_txtProveedor',0,Ubigeo,Direccion,'#MainContent_ddldireccionNueva','#MainContent_ddldireccionNuevaDestino',Codtipodoc);
        },    
        close: function () {
    ...
    $('#MainContent_txtTransportista').autocomplete({
     ...
     $('#MainContent_btnListarPedidos').click(function () {
    if (!F_SesionRedireccionar(AppSession)) return false;
    ...
    $('#MainContent_txtNumero').blur(function () {
        ...
        $('#MainContent_txtNotaPedido').blur(function () {
        var id = '00000000' + $('#MainContent_txtNotaPedido').val();
        $('#MainContent_txtNotaPedido').val(id.substr(id.length - 8));
        return false;
    });
    ...
    switch (Campos) {
        ...
        case "MainContent_txtVencimiento":
            ...
            break;
             ...
             case "MainContent_txtNotaPedido":
                        if ($("#MainContent_txtNotaPedido").val().trim() != "")
                            {
                                if (isNaN($("#MainContent_txtNotaPedido").val().trim()))
                                    $("#MainContent_txtNotaPedido").val('');
                                else
                ...
                function (result) {
                    ...
                    if (str_resultado_operacion == "1") {
                        ...
                        F_Update_Division_HTML('div_codunidadpeso', result.split('~')[19]);
                         F_Update_Division_HTML('div_NotaPedido', result.split('~')[20]);
                        ...
                        $('#MainContent_ddlNotaPedido').css('background', '#FFFFE0');
                       ...
                       $('#MainContent_txtNotaPedido').css('background', '#FFFFE0');
                       $('#hfDireccion').val('');
                       $('#hfDestino').val('');
...
function F_GrabarDocumento(){
    ...
    try 
        ...
        var objParams = {
                ...
                Filtro_Observacion: $('#MainContent_txtObservacionGuiaEdicion').val()
            };
...
//    $('#MainContent_txtNotaPedido').val($('#COTIZACION_NUMERO_' + Boton.split('_')[2]).val());
//    F_AgregarTemporalCTxNumero();
//}


function F_AgregarTemporalCTxBoton(Boton) {
    var indice = Boton.split('_')[2];
    
    
    var serie = $('#COTIZACION_SERIE_' + indice).val();
    ...
    var valorCombo = mapeoSeries[serie];
    
   
    if (valorCombo) {
        $('#MainContent_ddlNotaPedido').val(valorCombo).trigger('change');
    }
    
    
    $('#MainContent_txtNotaPedido').val($('#COTIZACION_NUMERO_' + indice).val());
    F_AgregarTemporalCTxNumero();
}
...
function F_AgregarTemporalCTxNumero() {
        ...
        if ($(Contenedor + 'txtNotaPedido').val() == "")
            Mensaje = Mensaje + "\n" + "Numero Cotizacion";
    ...
    $.ajax({
       ...
       url: 'GuiaRemisionUnitarioRoman.aspx/F_Obtener_Pedido_PorNumero_Net',

        dataType: "json",
        data: JSON.stringify({
            NumeroPedido: $(Contenedor + 'txtNotaPedido').val(),
             CodSerie: $(Contenedor + 'ddlNotaPedido option:selected').val()   //.text().trim()
             }),
            success: function (dbObject) {
            MostrarEspera(true);
            var data = dbObject.d;
...
//            console.log("data:", data);
       ...
       var tasaigv = parseFloat($("#MainContent_ddlIgv option:selected").text()) + parseFloat(1);
                    ...
                    //Array de Items
                    $.each(data.TrasladoDet, function (index, item) { //lleno del detallle
                        var objDetalle = {
                            ...
                            Dscto: 0,
                            CodDetalle: 0,
                            CodTrasladoDet: item.CodDetalle,
                ...
                //    console.table(arrDetalle);
                    ...
                    var objParams = {              //aca la cabecera de lo que devuelvo 
                        ...
                        Filtro_CodTraslado: data.CodTraslado,
                        Filtro_Igv: data.Igv,
                        Filtro_Total: data.Total,
                        Filtro_CodGuia: "0",
                        Filtro_Descuento: "0",
                        Filtro_TasaIgv: 1,
                        Filtro_TasaIgvDscto: 1,
                        Filtro_CodigoTemporal: $('#hfCodigoTemporal').val(),
                        Filtro_XmlDetalle: Sys.Serialization.JavaScriptSerializer.serialize(arrDetalle),
            ...
            //  $('#MainContent_lblNumRegistros').text(F_Numerar_Grilla("grvDetalleArticulo", 'lblProducto'));   
                 ...
                 //   alertify.log('Los Producto(s) se han agregado con exito');
                  ...
                  // $('#MainContent_txtProveedor').val(data.NroRuc);
                    ...
                    '#MainContent_txtProveedor',0,Ubigeo,Direccion,'#MainContent_ddldireccionNueva','#MainContent_ddldireccionNuevaDestino',1);
                     ...
                     BuscarDireccionCotizacion('#hfCodigoTemporal',data.CodDireccionDestino,'#MainContent_ddldireccionNuevaDestino','',23); 
...
function F_ReemplazarDocumento(Fila) {
    if (F_PermisoOpcion(CodigoMenu, CodigoInterno, 'Editar') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informacion
        try {
            ...
            var seriedoc = '#' + imgID.replace('imgReemplazar', 'lblNumero');
...
//            ,'#MainContent_txtProveedor',0,Ubigeo,Direccion,'#MainContent_ddldireccionNueva','#MainContent_ddldireccionNuevaDestino',1);
                 ...
                 BloqueoGuiaInterna($(Cuerpo + 'ddlTipoDocumento').val(),(Cuerpo + 'ddlTipoTransportista'),(Cuerpo + 'txtProveedor')
                 ,(Cuerpo + 'ddlMotivoTrabajo'),(Cuerpo + 'ImageButton1'),(Cuerpo + 'txtNroRucTransportista')                 
                 ,(Cuerpo + 'txtTransportista'),(Cuerpo + 'ddldireccionNuevaTransportista'),(Cuerpo + 'ImageButton2'),(Cuerpo + 'txtNuBultos')
                 ,(Cuerpo + 'txtPeso'),(Cuerpo + 'txtConductorDNI'),$(Cuerpo + 'txtConductorRazonSocial').val()
                 ,(Cuerpo + 'txtPlacaTraslado') ,(Cuerpo + 'txtLicenciaGuia'));

//original                    BuscarDireccionCotizacion('#hfCodigoTemporal', $('#hfCoddireccionDestino').val(),'#MainContent_ddldireccionNuevaDestino','');
...
function F_Pedidos() {
    ...
    $.ajax({
        ...
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        url: 'GuiaRemisionUnitarioRoman.aspx/F_Consulta_Pedido_Pendientes_NET',
        dataType: "json",
            ...
            try {
                ...
                $.each(data.rows, function (index, item) {
                    ...
                    var li = fmt2.replace('@Nro', Count).replace('@ID', 'COTIZACION_CODPROFORMA_' + Count).replace('@CodProforma', item.CodProforma)  
                                        .replace('@NroDocumento', String(item.Serie).padStart(4, '0') + '-' + Number(item.NumeroC))
                                        .replace("@ElVendedor", item.Vendedor)
...
// }; 
  if (APITransportista == "Usuario No Encontrado") {
        ...
        $.ajax({
            ...
            ,'#MainContent_txtTransportista',1,Ubigeo,direccion.replace(distrito, ""),'#MainContent_ddldireccionNuevaTransportista','',Codtipodoc);
                }
                catch (x) { }
                MostrarEspera(false);
            },
