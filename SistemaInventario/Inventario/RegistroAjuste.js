...
$(document).ready(function () {
    ...
    $('#MainContent_btnBuscar').click(function () {
        if (!F_SesionRedireccionar(AppSession)) return false;
        if ($('#MainContent_chkAjusteInventarioNegativoCero').is(':checked') | $('#MainContent_chkAjusteInventarioCero').is(':checked')) {
            toastr.warning('NO SE PUEDE BUSCAR PRODUCTOS CUANDO SE AJUSTE MASIVO A CERO');
            return false;
        }
    ...
    $('#MainContent_txtObservacionAjuste').css('background', '#FFFFE0');
    
    $('#MainContent_txtDesde').css('background', '#FFFFE0');

    $('#MainContent_txtHasta').css('background', '#FFFFE0');

    $('#MainContent_txtObservacionAjuste').prop('readonly', true)
...
function F_ValidarAgregar() {
    try {
        ...
        if ($('#MainContent_chkAjusteInventarioNegativoCero').is(':checked') | $('#MainContent_chkAjusteInventarioCero').is(':checked')) {
            if ($('#MainContent_txtObservacionAjuste').val().trim() == '') { 
                toastr.warning('INGRESAR OBSERVACION');
                return false;
            }
            else {
                return true;
...
//        var Operacion = 0;// AJUSTE NORMAL

//        if ($('#MainContent_chkAjusteInventarioNegativoCero').is(':checked'))
//            Operacion = 1; // AJUSTE SOLO DE NEGATIVOS A CERO

//        if ($('#MainContent_chkAjusteInventarioCero').is(':checked'))
//            Operacion = 2; // AJUSTE DE TODOS LOS PRODUCTOS A CERO
//        
//        $('#MainContent_grvConsultaArticulo .chkSi :checkbox').each(function () {
//            chkSi = '#' + this.id;
...
//            Filtro_DscProducto: $('#MainContent_txtArticulo').val(),
//            Filtro_Observacion: $('#MainContent_txtObservacionAjuste').val(),
...
//            if (str_resultado_operacion == "1") {
//                F_Update_Division_HTML('div_grvConsultaArticulo', result.split('~')[2]);
//                $('#MainContent_txtObservacionAjuste').val('');
//                $('#MainContent_txtObservacionAjuste').prop('readonly', true)
//                $('#MainContent_chkAjusteInventarioNegativoCero').prop('checked',false)
//                $('#MainContent_chkAjusteInventarioCero').prop('checked', false)
//                $('#MainContent_txtArticulo').focus();
//                $('.ccsestilo').css('background', '#FFFFE0');
//                toastr.warning(str_mensaje_operacion);                
//            } else {
...
function F_AgregarTemporal() {
    try {
        ...
        $('#MainContent_grvConsultaArticulo .chkSi :checkbox').each(function () {
            ...
            if ($(chkSi).is(':checked')) {
                var cantidad = parseFloat($(txtcantidad_grilla).val()) || 0;
                var stock = parseFloat($(lblstock_grilla).text()) || 0;
                var ajuste = cantidad - stock;
...
function F_NegativoCero(ControlID) {
    ...
    if ($(chkok_grilla).is(':checked')) {
        $('#MainContent_chkAjusteInventarioCero').prop('checked', false);
        $('#MainContent_txtObservacionAjuste').prop('readonly', false);
    }
    else {
        $('#MainContent_txtObservacionAjuste').prop('readonly', true);
        $('#MainContent_txtObservacionAjuste').val('');
...
function F_LlenarGridDetalle(Fila) {
    try {
        ...
        var Codigo = $('#' + Fila.replace('pnlOrders', 'hfCodigoTransaccion')).val();
...
function F_Ini_ComboUsuario() {
    ...
    $.ajax({
        ...
        url: '/Maestros/Perfiles.aspx/F_Usuario_Listar_NET',
        ...
        success: function (dbObject) {
            ...
            try {
                ...
                $('#MainContent_ddlUsuario').append($("<option></option>").val(0).html('TODOS'));
...
function F_Buscar() {
    ...
    try {
        ...
        var objParams = {
            Filtro_CodUsuario: $("#MainContent_ddlUsuario").val(),         
        ...
        F_Buscar_NET(arg, function (result) {
            ...
            if (str_resultado_operacion == "1") {
                ...
                $('#lblGrillaConsulta').text(F_Numerar_Grilla("grvConsulta", 'lblFecha'));
...
function F_EliminarRegistro(Fila) {
    ...
    try {
        var imgID = Fila.id;
        var hfCodigoTransaccion = '#' + imgID.replace('imgAnularDocumento', 'hfCodigoTransaccion');

        if (!confirm("ESTA SEGURO DE ELIMINAR EL AJUSTE DE INVENTARIO"))
            return false;
        ...
        var objParams = {
            Filtro_Codigo: $(hfCodigoTransaccion).val(),         
            Filtro_Desde: $('#MainContent_txtDesde').val(),
            Filtro_Hasta: $('#MainContent_txtHasta').val(),         
            Filtro_ChkFecha: chkFecha         
        };
