var AppSession = "../Ventas/RegistroFacturaMultipleKuyay.aspx";
var Codtipodoc=1
var CodigoMenu = 4000;
var CodigoInterno = 2;
var CodCategoria = 1;
...
//                        $('#hfPartida').val(result.split('~')[11]);
                        ...
                        $('#MainContent_txtAtencion').val('');
                        $('#MainContent_txtLote').css('background', '#FFFFE0');
...
return true;
 ...
 try 
        ...
        var lote = $('#MainContent_txtLote').val();
        var txtFechaCaducidad= '';
        var txtLote= '';
        ...
        if ($("#MainContent_txtCantidad").val() == '')
         ...
         if ($("#MainContent_txtLote").val().trim() == '')
            Cadena = Cadena + '<p></p>' + 'Lote';


        if ($("#MainContent_txtLote").val()=='')
           cadena = cadena + "CAMPO Lote <p></p>"

        if ($("#MainContent_txtFechaCaducidad").val()=='')
           cadena = cadena + "CAMPO FECHA CADUCIDAD <P></P>"



        var fechaCaducidad = $("#MainContent_txtFechaCaducidad").val();

        if (fechaCaducidad === '') {
            cadena = cadena + "CAMPO FECHA CADUCIDAD <P></P>";
        } else {
             
            var partes = fechaCaducidad.split('/');
            ...
            if (fechaIngresada < hoy) {
                cadena = cadena + "La fecha de vencimiento debe ser mayor a la de hoy <P></P>";
            }
        }
...
//        }
        ...
        PermisoCambio = false;
                if (cadena != "Ingrese los sgtes. campos:  <p></p> ")
                   ...
                   {
                    cadena="Los sgtes. productos tienen el mismo lote :<p></p> ";
...
//                            {
                          ...
                          $('#MainContent_grvDetalleArticulo .chkDelete :checkbox').each(function () {
                                    ...
                                    hfcodarticulodetalle_grilla = chkDel.replace('chkEliminar', 'hfcodarticulo');
                                    lbldscproducto_grilla = chkDel.replace('chkEliminar', 'lblproducto');
                                    var hflote_grilla = chkDel.replace('chkEliminar', 'hfLoteDetalle'); 
                                    var lote = $('#MainContent_txtLote').val(); 

                                    if (
                                        $('#hfCodProductoAgregar').val() === $(hfcodarticulodetalle_grilla).val() &&
                                        lote === $(hflote_grilla).val()
                                    ) {
                                        cadena = cadena + "\n" + $('#MainContent_txtArticuloAgregar').val() + "  " + lote;
                                        F_TablaDown();
                                    }
                                });
...
//                    });
                   ...
                   if (cadena != "Los sgtes. productos tienen el mismo lote :<p></p> ") 
                   {alertify.log(cadena);
                   return false;}
                   else
                   {
...
//        }
        ...
        Vencimiento : $('#MainContent_txtFechaCaducidad').val(),
        Lote: $('#MainContent_txtLote').val(),
        CodDetalle: 0,
        Acuenta: 0,
        CodTipoDoc: 0,
        Filtro_FlagIgv: FlagIgv,
...
function F_ActualizarPrecio(Fila) {
    try {
        ...
        var txtFechaVencimiento = txtPrecio.replace('txtPrecio','txtFechaVencimiento');
        var txtLote= txtPrecio.replace('txtPrecio','txtLote');
...
//        }
        ...
        var objParams = {
            ...
            Filtro_FechaVencimiento:$(txtFechaVencimiento).val(),
            Filtro_Lote : $(txtLote).val(),
            Filtro_TasaIgv: tasaigv,
            Filtro_NotaPedido: 0,
            Filtro_FlagIgv: FlagIgv,
            Filtro_Flag: 0,
...
function F_ActualizarCantidad(Fila) {
    try {
        ...
        var txtFechaVencimiento = txtCantidad.replace('txtCantidad', 'txtFechaVencimiento');
        var txtLote = txtCantidad.replace('txtCantidad', 'txtLote');
...
function F_ActualizarVencimiento(Fila) {
    try {
        var txtFechaVencimiento = '#' + Fila;
        var hfCodDetalle = txtFechaVencimiento.replace('txtFechaVencimiento', 'hfCodDetalle');
        var txtPrecio = txtFechaVencimiento.replace('txtFechaVencimiento', 'txtPrecio');
        var hfPrecio = txtFechaVencimiento.replace('txtFechaVencimiento', 'hfPrecio');
        var txtCantidad = txtFechaVencimiento.replace('txtFechaVencimiento', 'txtCantidad');
        var hfCantidad = txtFechaVencimiento.replace('txtFechaVencimiento', 'hfCantidad');
        var txtDescripcion = txtFechaVencimiento.replace('txtFechaVencimiento', 'txtDescripcion');
        var hfExclusivo = txtFechaVencimiento.replace('txtFechaVencimiento', 'hfExclusivo');
        var txtLote = txtFechaVencimiento.replace('txtFechaVencimiento', 'txtLote');
        var FlagIgv = 0;
...
function F_ActualizarLote(Fila){
 try{
    var txtLote = '#' + Fila;
    var hfCodDetalle = txtLote.replace('txtLote', 'hfCodDetalle');
    var txtPrecio = txtLote.replace('txtLote', 'txtPrecio');
    var hfPrecio = txtLote.replace('txtLote', 'hfPrecio');
    var txtCantidad = txtLote.replace('txtLote', 'txtCantidad');
    var hfCantidad = txtLote.replace('txtLote', 'hfCantidad');
    var txtDescripcion = txtLote.replace('txtLote', 'txtDescripcion');
    var txtFechaVencimiento = txtLote.replace('txtLote','txtFechaVencimiento');
    var hfExclusivo = txtLote.replace('txtLote', 'hfExclusivo');
    var FlagIgv= 0;
...
function F_ActualizarDescripcion(Fila) {
    try {
        ...
        var txtFechaVencimiento = txtDescripcion.replace('txtDescripcion', 'txtFechaVencimiento');
        var txtLote = txtDescripcion.replace('txtDescripcion','txtLote');
...
function F_InicializarCajaTexto()
{
    ...
    $("#MainContent_txtFechaCaducidad").css('background', '#FFFFE0');
...
function F_FacturaGuia() {
    ...
    try {
        ...
        F_FacturacionGuia_NET(arg, function (result) {
            ...
            ,'#MainContent_txtTransportista',1,result.split('~')[9],result.split('~')[28],'#MainContent_ddldireccionNuevaTransportista','',1);
            }
