...
function F_Controles_Inicializar() {
            ...
            (
                ...
                function (result) {
                    ...
                    if (str_resultado_operacion == "1") 
                    {
                        ...
                        F_Update_Division_HTML('div_VendedorEdicion', result.split('~')[30]);
                        F_Update_Division_HTML('div_TipoTransportista', result.split('~')[25]);
                        F_Update_Division_HTML('div_codunidadpeso', result.split('~')[27]);
                        F_Update_Division_HTML('div_TipoTransportistaEdicion', result.split('~')[26]);
                        F_Update_Division_HTML('div_codunidadpesoedicion', result.split('~')[28]);
    ...
    $('#MainContent_txtConductorDNI').css('background', '#FFFFE0');
                       ...
                       $('#MainContent_txtObservacionGuiaEdicion').css('background', '#FFFFE0');
                        ...
                        //F_Mostrar_CorrelativoVarios(1);
                        $('.ccsestilo').css('background', '#FFFFE0');          
                $('.ccsestilo2').css('background', '#FFFFE0');
                        F_CambioTipo();
...
function F_AgregarArticulo(ControlID, DirectoBoton) {
    ...
    hfstock = imgAgregar_grilla.replace('imgAgregar', 'lblstock');
    hfstockcieneguilla = imgAgregar_grilla.replace('imgAgregar', 'lblStockcieneguilla');
    ...
    hfMargenStockTransportista= imgAgregar_grilla.replace('imgAgregar', 'hfMargenStockTransportista');
    hfStockTransportista = imgAgregar_grilla.replace('imgAgregar', 'hfStockTransportista');
    ...
    $('#hfMargenStockTransportista').val($(hfMargenStockTransportista).val());
    $('#hfStockTransportista').val($(hfStockTransportista).val());
    ...
    $('#hfstock').val($(hfstock).text());
    $('#hfstockcieneguilla').val($(hfstockcieneguilla).text());
...
function F_AgregarTemporal() {
    ...
    try {
        ...
        if (F_PermisoOpcion_SinAviso(CodigoMenu, 777002, '') === "0") { 
        ...
        AgregandoProducto = false;
            ...
            if (str_resultado_operacion == "1") {
                $('#hfCodigoTemporal').val(result.split('~')[3]);
                $('#MainContent_lblID').text(result.split('~')[3]);
        ...
        if (cantidadArticulos === 0)
            ...
            switch($("#MainContent_ddlTipoDoc").val()) {
             ...
             if ($(Cuerpo + 'chkGuia').is(':checked')) {         
                 ...
                 if ($('#MainContent_ddlTipoTransportista').val()==0)               
                     Cadena = Cadena  + '<p></p>' + 'SELECCION TIPO TRANSPORTISTA';                                                   
             } 
...
function F_EditarRegistro(Fila) {
 ...
 try 
    ...
    F_DatosFactura_NET(arg, function (result) {
                    ...
                    $('#hfDestinoEdicion').val(result.split('~')[13]);
                    ...
                    $('#MainContent_txtobservacionEditar').val(result.split('~')[44]);
                    ...
                    $('#MainContent_txtConductorDNIEdicion').val(result.split('~')[38]);
                    $('#MainContent_txtConductorRazonSocialEdicion').val(result.split('~')[39]);
                    $('#hfCodConductor').val(result.split('~')[33]);
                    $('#MainContent_txtNroRucTransportistaEdicion').val(result.split('~')[35]);  
                    ...
                    if ($('#hfCodTransportista').val()>0 || $('#hfCodConductor').val()>0)
                    {
                        ...
                        BuscarDireccionCotizacion('#hfCodigoTemporalEdicion',result.split('~')[41],'#MainContent_ddldireccionNuevaDestinoEdicion','',Codtipodoc);
                    }
                    else
                    {
...
//          si el usuario no esta autorizado para modificar precios
         if (F_PermisoOpcion_SinAviso(CodigoMenu, 777002, '') === "0") { 
             if (parseFloat($(txtPrecio).val()) < parseFloat($(hfP1).val()) ) {
                xSolicitarDescuento = 1;
                alertify.log("SE HA ENVIADO SOLICITUD DE DESCUENTO AL SUPERVISOR, SI NO LO DESEA, VUELVA AL PRECIO ANTERIOR");
            }
        }
...
function F_ActualizarCantidad(Fila) {
    try {
        ...
        //si el usuario no esta autorizado para modificar precios
         if (F_PermisoOpcion_SinAviso(CodigoMenu, 777002, '') === "1") { 
        ...
        var objParams = {
            ...
            Filtro_PrecioDscto: $(txtPrecio).val(),
            Filtro_Cantidad: $(txtCantidad).val(),
            Filtro_CodigoTemporal: $('#hfCodigoTemporal').val(),
            Filtro_Descripcion: $(txtDescripcion).val(),
            Filtro_CodDetDocumentoVenta: $(hfCodDetalle).val(),
...
function F_AgregarTemporalNV() {
    try {
        ...
        $('#MainContent_grvDetalleNV .chkDelete :checkbox').each(function () {
            ...
            hfCodCliente = chkSi.replace('chkEliminar', 'hfCodCliente');
            hfCliente = chkSi.replace('chkEliminar', 'hfCliente');

            if ($(chkSi).is(':checked')) {
                $('#MainContent_txtNroRuc').val($(hfNroRuc).val());
                $('#hfCodCtaCte').val($(hfCodCliente).val());
                $('#MainContent_txtCliente').val($(hfCliente).val());
                F_ValidaRucDni();
...
function F_ActualizarDescuento(Fila) {
    try {
        ...
        var objParams = {
            ...
            Filtro_CodigoTemporal: $('#MainContent_lblID').text(),
            Filtro_CodMoneda: $('#MainContent_ddlMoneda').val(),
            Filtro_TasaIgv: tasaigv,
            Filtro_NotaPedido: 0,
            Filtro_FlagIgv: FlagIgv,
...
var D=0;
                        ...
                        var txtDescripcion = '#' + this.id;
                        var lblCodigoProducto = txtDescripcion.replace("txtDescripcion", "lblCodigoProducto");
                        var lblMarca = txtDescripcion.replace("txtDescripcion", "lblMarca");
                        var txtCantidad = txtDescripcion.replace("txtDescripcion", "txtCantidad");
                        var lblUM = txtDescripcion.replace("txtDescripcion", "lblUM");
                        var txtPrecio = txtDescripcion.replace('txtDescripcion', 'txtPrecio');
                        var lblImporte = txtDescripcion.replace('txtDescripcion', 'lblImporte');
                        var hfP1 = txtDescripcion.replace('txtDescripcion', 'hfP1');
                        var hfDescuentoAprobado = txtDescripcion.replace('txtDescripcion', 'hfDescuentoAprobado');
                        var hfSolicitudDescuento = txtDescripcion.replace('txtDescripcion', 'hfSolicitudDescuento');
                          D++             
                        if (parseFloat($(hfDescuentoAprobado).val()) == 0 &&  parseFloat($(hfSolicitudDescuento).val()) == 1 ) {
                            $(txtDescripcion).css("color", "Violet");
                            $(lblCodigoProducto).css("color", "Violet");
                            $(lblMarca).css("color", "Violet");
                            $(txtCantidad).css("color", "Violet");
                            $(lblUM).css("color", "Violet");
                            $(txtPrecio).css("color", "Violet");
                            $(lblImporte).css("color", "Violet");
                        } else if (parseFloat($(hfDescuentoAprobado).val()) == 2 &&  parseFloat($(hfSolicitudDescuento).val()) == 1 ){
                            $(txtDescripcion).css("color", "red");
                            $(lblCodigoProducto).css("color", "red");
                         ...
                         $(txtDescripcion).css("color", "blue");
                            $(lblCodigoProducto).css("color", "black");
                            $(lblMarca).css("color", "black");
                            $(txtCantidad).css("color", "blue");
                            $(lblUM).css("color", "black");
...
function F_ImprimirFacturaGrilla(Fila,CodMenu) {
       ...
       alertify.log("No tiene guia adjunta");
        ...
        {
            Codigo = $('#' + imgID.replace('imgPdf', 'lblCodigo')).val() ; 
            Estado = $('#' + imgID.replace('imgPdf', 'lblEstado')).val() ;       
        }       
