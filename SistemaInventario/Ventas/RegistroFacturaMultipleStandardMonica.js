var AppSession = "../Ventas/RegistroFacturaMultipleStandardMonica.aspx";
var Codtipodoc=1
var CodigoMenu = 4000;
var CodigoInterno = 2;
var CodCategoria = 1;
...
$(document).ready(function () {
   ...
   // F_AUTOCOMPLETECONDUCTORDNIEDICION();
    ...
    F_AUTOCOMPLETEPLACA_1();
    F_AUTOCOMPLETEPLACA_CONSULTA();
    ...
    $('#MainContent_txtPrecioDisplay').blur(function () {
    if ((parseFloat($('#hfstock').val())+parseFloat($('#hfstockcieneguilla').val()))<parseFloat($('#hfStockTransportista').val()))
    {
     if (parseFloat($('#MainContent_txtPrecioDisplay').val())<parseFloat($('#hfMenorPrecioAgregar').val()))
     {
     $('#MainContent_txtPrecioDisplay').val($('#hfMenorPrecioAgregar').val());
     alertify.log('NO SE PUEDE PONER UN MONTO MENOR AL MINIMO SI SU STOCK TRANSPORTISTA ES MENOR AL STOCK');
     }
    }

    if(((parseFloat($('#hfstock').val())+parseFloat($('#hfstockcieneguilla').val()))-parseFloat($('#MainContent_txtCantidad').val()))<(parseFloat($('#hfStockTransportista').val())*(parseFloat($('#hfMargenStockTransportista').val())/100)))
    {
    if (parseFloat($('#MainContent_txtPrecioDisplay').val())<parseFloat($('#hfMenorPrecioAgregar').val()))
     {
     $('#MainContent_txtPrecioDisplay').val($('#hfMenorPrecioAgregar').val());
...
//    $('#MainContent_txtCantidad').enterKey(function () {
...
//    if (((parseFloat($('#hfstock').val())+parseFloat($('#hfstockcieneguilla').val()))-parseFloat($('#MainContent_txtCantidad').val()))<parseFloat($('#hfStockTransportista').val()))
//    {
    ...
    $("#MainContent_chkGuia").click(function () {
        if ($(this).is(':checked'))
        {
          BloqueoTipoTransportista($(Cuerpo + 'ddlSerieGuia option:selected').text(),(Cuerpo + 'txtNumeroGuia'),(Cuerpo + 'txtFechaTraslado')
                 ...
                 ,(Cuerpo + 'txtMarcaGuia'),
                 $(Cuerpo + 'ImageButton1'),
                 $(Cuerpo +'ImageButton2'),'#MainContent_chkGuia','#hfCodTransportista');
...
//        if (F_PermisoOpcion(CodigoMenu, 4000601, '') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informac
        ...
        var Contenedor = '#MainContent_';
        $(Contenedor + 'txtGuia').val('');
        $('#div_GuiaConsignacion').dialog('open');

        return false;

    });
        
    $('#MainContent_btnFacturarGuia').click(function () {
        if (!F_SesionRedireccionar(AppSession)) return false;
        try {
            F_FacturaGuia();
            return false;
        ...
        $('#MainContent_btnGrabarMotor').click(function () {
        if (!F_SesionRedireccionar(AppSession)) return false;   
        try {
            if (confirm("ESTA SEGURO DE LA ACTUALIZACION"))
                F_EditarMotor();
...
$(document).on("change", "select[id $= 'MainContent_ddlTipoTransportistaEdicion']",function () {
     BloqueoTipoTransportista($(Cuerpo + 'ddlSerieGuiaEdicion option:selected').text(),(Cuerpo + 'txtNumeroGuiaEdicion'),(Cuerpo + 'txtFechaTrasladoEdicion')
                 ...
                 (Cuerpo + 'txtTransportistaEdicion'),(Cuerpo + 'txtNroRucTransportistaEdicion')
                 ,(Cuerpo + 'txtPlacaTrasladoEdicion'),(Cuerpo + 'txtLicenciaGuiaEdicion'),(Cuerpo + 'txtNuBultosEdicion'),(Cuerpo + 'txtPesoEdicion')
                 ,(Cuerpo + 'txtConductorRazonSocialEdicion'),(Cuerpo + 'txtConductorDNIEdicion'),$(Cuerpo + 'ddlTipoTransportistaEdicion').val()
                 ,(Cuerpo + 'txtDistritoTransportistaEdicion'),(Cuerpo + 'txtDireccionTransportistaEdicion')
                 ,$('#hfCodDistritoTransportistaEdicion').val(),$('#hfCodDireccionDefectoTransportista').val(),$('#hfCodConductor').val(),(Cuerpo + 'chkGuiaEdicion'));
});
...
if (!F_SesionRedireccionar(AppSession)) return false;
    ...
    Codtipodoc=$("#MainContent_ddlTipoDoc2").val()
            ...
            (
                ...
                function (result) {
                    ...
                    if (str_resultado_operacion == "1") 
                    {
                        ...
                        F_Update_Division_HTML('div_TipoTransportista', result.split('~')[29]);
                        F_Update_Division_HTML('div_codunidadpeso', result.split('~')[31]);
                        F_Update_Division_HTML('div_TipoTransportistaEdicion', result.split('~')[30]);
                        F_Update_Division_HTML('div_codunidadpesoedicion', result.split('~')[32]);
                        F_Update_Division_HTML('div_Almacen', result.split('~')[33]);
...
//                        $('#hfPartida').val(result.split('~')[11]);
                        ...
                        $('#MainContent_chkWhatsApp').prop('checked', true);
                       ...
                       $("#MainContent_txtExonerada").css('background', '#FFFFE0');
                        //Placas
            ...
            (
                ...
                function (result) {
                    ...
                    if (str_resultado_operacion == "1") {
                        ...
                        F_Grilla_Stock_Color('grvConsultaArticulo','lblProducto','lblstock',' .detallesart');    
...
function F_AgregarArticulo(ControlID, DirectoBoton) {
    ...
    var hfCategoriaIgv_grilla = imgAgregar_grilla.replace('imgAgregar', 'hfCategoriaIgv');
    $('#hfCategoriaIgvAgregar').val($(hfCategoriaIgv_grilla).val());
...
//           var CategoriaIgv = $('#hfCategoriaIgvAgregar').val();
//           if (CategoriaIgv == "3") {
//           tasaigv = 1;
//           FlagIgv = 0;
//                }

            // Calculamos la tasa IGV específica del producto a agregar
            var CategoriaIgv = $('#hfCategoriaIgvAgregar').val();
            var tasaigv_producto = 1;
            var flagigv_producto = 0;

            if ($('#MainContent_chkConIgvMaestro').is(':checked') && CategoriaIgv != "3") {
                tasaigv_producto = parseFloat($("#MainContent_ddlIgv option:selected").text()) + 1;
                flagigv_producto = 1;
            }
...
//        CodTipoDoc: 0,
//        //Filtro_FlagIgv: FlagIgv,
//        Filtro_FlagIgv: flagigv_producto,
//        Filtro_Flag: 0,
//        //Filtro_TasaIgv: tasaigv,
//        Filtro_TasaIgv: tasaigv_producto,
//        Filtro_TasaIgvDscto: parseFloat($("#MainContent_ddlIgv option:selected").text()) + parseFloat(1)  ,
//        Filtro_TipoOperacion:1,
//        CodCategoriaIgv:$('#hfCategoriaIgvAgregar').val()
//        };
//        arrDetalle.push(objDetalle);


                var precioBase = parseFloat($('#MainContent_txtPrecioDisplay').val());
        if (CategoriaIgv == "3") {
            // Exonerado: no se divide, el precio es tal cual
            var PrecioCalc = precioBase;
        } else {
            // Afecto: si está con IGV, se divide
            var PrecioCalc = precioBase / tasaigv_producto;
        }
        ...
        var objDetalle = {
            ...
            CantidadOC: 0,
            Precio: PrecioCalc,
            PrecioDscto: PrecioCalc,
            Precio2: Precio2 / tasaigv_producto,
            Exclusivo: parseFloat($('#MainContent_txtExclusivo').val()) / tasaigv_producto,
            Costo: $('#hfCostoAgregar').val(),
            CodUm: $('#hfCodUmAgregar').val(),
            Descripcion: $('#MainContent_txtArticuloAgregar').val(),
            CodDetalle: 0,
            ...
            Filtro_FlagIgv: flagigv_producto,
            Filtro_TasaIgv: tasaigv_producto,
            Filtro_TasaIgvDscto: parseFloat($("#MainContent_ddlIgv option:selected").text()) + 1,
            Filtro_TipoOperacion: 1,
            CodCategoriaIgv: CategoriaIgv
        }; arrDetalle.push(objDetalle);
        ...
        F_EliminarTemporal_NET(arg, function (result) {
            ...
            if (str_resultado_operacion == "1") {
                ...
                if (result.split('~')[5] == '0') {
                    ...
                    $('#MainContent_txtSubTotal').val(parseFloat(result.split('~')[7]).toFixed(2));
                    $('#MainContent_txtAcuentaNV').val(parseFloat(result.split('~')[8]).toFixed(2));
                            $('#MainContent_txtExonerada').val(result.split('~')[10]);
                }
...
function F_ValidarGrabarDocumento(){
    try 
        {      
           var Cuerpo='#MainContent_';
           var Cadena=F_ValidarFacturaVenta_General($(Cuerpo + 'txtNroRuc').val(),$(Cuerpo + 'txtCliente').val(),$('#hfCodCtaCte').val(),$("#MainContent_ddlTipoDoc").val(),
                      $(Cuerpo + 'lblTC').text(),$(Cuerpo + 'ddlVendedor').val(),$(Cuerpo + 'txtNumero').val(),$(Cuerpo + 'txtEmision').val(),$(Cuerpo + 'txtNroOperacion').val(),
                      $(Cuerpo + 'ddlFormaPago').val(),$(Cuerpo + 'txtTotal').val(),$(Cuerpo + 'txtCorreo').val(),$('#MainContent_ddldireccionNueva').val(),$('#MainContent_ddlMoneda').val());
        
           if ($(Cuerpo + 'chkGuia').is(':checked')) {         
                 if(($(Cuerpo + 'ddlSerieGuia option:selected').text()).substr(0, 1)=='T' && $('#MainContent_ddlTipoDoc').val()!=16 ){

                 var Cadena=Cadena   + Validacionguia($(Cuerpo + 'ddlSerieGuia option:selected').text(),$(Cuerpo + 'txtNumeroGuia').val(),$(Cuerpo + 'txtFechaTraslado').val(),
                            $(Cuerpo + 'ddldireccionNuevaDestino').val(),$(Cuerpo + 'txtTransportista').val(),$(Cuerpo + 'txtNroRucTransportista').val(),$(Cuerpo + 'txtPlacaTraslado').val(),
                            $(Cuerpo + 'txtLicenciaGuia').val(),$(Cuerpo + 'txtNuBultos').val(),$(Cuerpo + 'txtPeso').val(),$(Cuerpo + 'txtConductorRazonSocial').val(),
                            $(Cuerpo + 'txtConductorDNI').val(),$(Cuerpo + 'ddlTipoTransportista').val(),$(Cuerpo + 'ddldireccionNuevaTransportista').val());
                 }    
           ...
           if (F_ValidarCelular($(Cuerpo + 'txtCelular').val())!='')
               Cadena=Cadena + '<p></p>' + F_ValidarCelular($(Cuerpo + 'txtCelular').val());
...
function F_GrabarDocumento(){
  try 
        ...
        var FlagConCodigo=0; if ($(Contenedor + 'chkConCodigo').is(':checked'))   FlagConCodigo=1;
        var FlagGuia=0; if ($(Contenedor + 'chkGuia').is(':checked'))   FlagGuia=1;
        var Celular = '';
        ...
        $('#MainContent_grvDetalleArticulo .chkDelete :checkbox').each(function () {
               chkSi = '#' + this.id;
               if ($(chkSi.replace('chkEliminar', 'hfCodTipoDoc')).val()==10 || $(chkSi.replace('chkEliminar', 'hfCodTipoDoc')).val()==16)
               {
               $('#hfNotaPedido').val(1);
               }         
        });
        ...
        var objParams = {
            ...
            Filtro_Placa1:$(Contenedor + 'txtPlaca1').val(),
            ...
            Filtro_OrdenCompra: $('#MainContent_txtNroOC').val(),
            ...
            Filtro_Exonerada: $(Contenedor + 'txtExonerada').val(),
            ...
            // FIN DATOS DE LA GUIA         
                ...
                if (str_resultado_operacion == "1") 
                {
                    if (str_mensaje_operacion=='Se Grabo Correctamente')
                    {
                        ...
                        var CodMenu = 201; var FlagWhatsApp = 0;                                                             
...
function F_Nuevo(){
       ...
       $('#MainContent_txtSubTotal').val('0.00');
       $('#MainContent_txtGratuito').val('0.00');
       $('#MainContent_txtExonerada').val('0.00');
       $('#MainContent_txtIgv').val('0.00');
       $('#MainContent_txtTotal').val('0.00');
       $('#MainContent_txtComisionTarjeta').val('0.00');
       $('#MainContent_txtVencimiento').val($('#MainContent_txtEmision').val());
       ...
       try 
        {
              ...
              var objParams = {
                                ...
                                Filtro_CodAlmacenGeneral: $('#MainContent_ddlAlmacen').val(),
                ...
                {                  
                  ...
                  if (F_PermisoOpcion_SinAviso(CodigoMenu, 4000206, '') === "1") {   
...
//                        PRUEBA
                    if(typeof $(PTOTAL +  (index )).val() !=='undefined'){
                       ...
                       $('#lblTotalSoles').css("font-size","x-large"); 
                       $('#lblTotalDolares').css("font-size","x-large");
                       $("#CeldaTotales").prop("style").display="block";
                  }
                  else {
                  $("#CeldaTotales").prop("style").display="none";
                  }
 ...
 try 
        ...
        F_DatosFactura_NET(arg, function (result) {
                    ...
                    if (str_resultado_operacion == "1") {                       
                        ...
                        F_Habilitar_Controles_Edicion_Guia(result);
...
var arrDetalle = new Array();
    ...
    var objParams = {
                      ...
                      Filtro_FechaVencimiento:  $('#MainContent_txtVencimientoEdicion').val(),
                      ...
                      Filtro_OrdenCompra:$('#MainContent_txtNroOCEdicion').val(),
                      ...
                      Filtro_Celular: $('#MainContent_txtcelularEdicion').val(),
...
function F_ActualizarPrecio(Fila) {
    try {
        ...
        var FlagIgv = 0;
       // var hfCategoriaIgv = txtPrecio.replace('txtPrecio', 'hfCategoriaIgv');
        var hfCategoriaIgv = txtCantidad.replace('txtCantidad', 'hfCodCategoriaIgvGR');
     ...
     if (parseInt($(hfCodTipoDoc).val())==16 & parseFloat($(txtPrecio).val())!=parseFloat($(hfPrecio).val())  & parseFloat($(txtPrecio).val())!=0)
            {
...
//            if (parseFloat($(hfCosto).val())>=parseFloat($(txtPrecio).val()) & parseFloat($(txtPrecio).val())!=0)
//            {
//             $(txtPrecio).val(parseFloat($(hfPrecio).val()).toFixed(2));
...
//        }
        ...
        if (parseFloat($(txtPrecio).val()) == parseFloat($(hfPrecio).val()) & parseFloat($(txtCantidad).val()) == parseFloat($(hfCantidad).val())) {
           ...
           var CategoriaIgv = $(hfCategoriaIgv).val();
           var precioDisplay = parseFloat($(txtPrecio).val());
           var precioBase = precioDisplay;
           var tasaigv_producto = 1;
           var flagigv_producto = 0;

           var tasaigv_producto = parseFloat($("#MainContent_ddlIgv option:selected").text()) + 1;
 
            // no dividir el precio y enviar tasa = 1 y flag = 0
            if (CategoriaIgv == "3") {
                // Producto exonerado: el precio NO se divide
                precioBase = precioDisplay;
             // tasaigv_producto = 1;
             var tasaigv_producto = parseFloat($("#MainContent_ddlIgv option:selected").text()) + 1;
                 flagigv_producto = 0;
            } 
            // Si no es exonerado y el documento trabaja con IGV
            else if ($('#MainContent_chkConIgvMaestro').is(':checked')) {
                // Producto afecto con IGV: dividir el precio
                tasaigv_producto = parseFloat($("#MainContent_ddlIgv option:selected").text()) + 1;
                precioBase = precioDisplay / tasaigv_producto;
                flagigv_producto = 1;
            }
...
//        }   
           ...
           // Filtro_Precio: $(txtPrecio).val() / tasaigv,
            Filtro_Precio :   precioBase,
            //Filtro_Precio2: PrecioLimite / tasaigv,
            Filtro_Precio2: PrecioLimite / tasaigv_producto,
...
function F_ActualizarCantidad(Fila) {
    try {
        ...
        var hfCodTipoDoc = txtCantidad.replace('txtCantidad', 'hfCodTipoDoc');
        var Flag = 0;
        var FlagIgv = 0;
      //  var hfCategoriaIgv = txtCantidad.replace('txtCantidad', 'hfCategoriaIgv');
        var hfCategoriaIgv = txtCantidad.replace('txtCantidad', 'hfCodCategoriaIgvGR');
        ...
        if (parseFloat($(txtPrecio).val()) == parseFloat($(hfPrecio).val()) & parseFloat($(txtCantidad).val()) == parseFloat($(hfCantidad).val())) {
            ...
            return false;
              ...
              if (parseInt($(hfCodTipoDoc).val())==16 & parseFloat($(txtCantidad).val())>parseFloat($(hfCantidadOC).val()) )
            {
             $(txtCantidad).val(parseFloat($(hfCantidad).val()).toFixed(2));
             alertify.log("LA CANTIDAD NO PUEDE SER SUPERIOR A LA CANTIDAD REGISTRADA EN LA NOTA DE VENTA");              
            return false;
            }
...
//            // 
//            else {
//              // El precio mostrado ya es el precio base
//                precioBase = precioDisplay;
//              //  tasaigv_producto = 1;
//                flagigv_producto = 0;
//            }
...
function F_ValidarAgregarNV() {
    try {
        ...
        $('#MainContent_grvDetalleNV .chkDelete :checkbox').each(function () {
            ...
            if ($(chkSi).is(':checked')) {
                ...
                x = 1;
                 ...
                 $('#MainContent_grvDetalleNV .chkDelete :checkbox').each(function () {
                    chkgrvDetalleNV = '#' + this.id;
                    lblCodDetalle = chkgrvDetalleNV.replace('chkEliminar', 'lblCodDetalle');
               
                         if ($(chkgrvDetalleNV).is(':checked')) 
                            {
                                 $('#MainContent_grvDetalleArticulo .chkDelete :checkbox').each(function () {
                                    chkgrvDetalleArticulo = '#' + this.id;
                                    lblCodigo    = chkgrvDetalleArticulo.replace('chkEliminar', 'lblCodigo');
                                    hfCodDetalleOC = chkgrvDetalleArticulo.replace('chkEliminar', 'hfCodDetalleOC');
                                    if ($(hfCodDetalleOC).val()==$(lblCodDetalle).val())
                                    {
                                        cadena = cadena + "<p></p>" + $(lblCodigo).text();
                                        j+=1;
                                    }
                                 });
                            }
        ...
        if (j > 0)
            cadena = 'LOS SGTES CODIGOS SE ENCUENTRAN AGREGADOS'  + "<p></p>" + cadena;
...
//                    Dscto: 0,
//                    CodDetalle: 0,
//                    CodDetDocumentoVenta: $(lblCodDetalle).val(),
//                    CodProformaDet: 0,
//                    OC: $(lblNumero).text(),
...
function F_AgregarTemporalNV() {
    try {
        ...
        var Contenedor = '#MainContent_';
        var tasaigv = parseFloat($("#MainContent_ddlIgv option:selected").text()) + parseFloat(1);
         var tasaigv_maestro = parseFloat($("#MainContent_ddlIgv option:selected").text()) + 1;
        ...
        $('#MainContent_grvDetalleNV .chkDelete :checkbox').each(function () {
            ...
            hfCategoriaIgv = chkSi.replace('chkEliminar', 'hfCategoriaIgv');
            ...
            if ($(chkSi).is(':checked')) {           
                ...
                var CategoriaIgv = $(hfCategoriaIgv).val() ;
                var precioDisplay = parseFloat($(hfPrecioSinIgv).val()) ;
                var cantidad = parseFloat($(txtcantidad_grilla).val()) ;
                ...
                if ($('#MainContent_chkConIgvMaestro').is(':checked') && CategoriaIgv !== "3") {
                    tasaigv_producto = tasaigv_maestro;
                    flagigv_producto = 1;
                }
        ...
        var objParams = {
           ...
           // Filtro_TasaIgv: parseFloat($("#MainContent_ddlIgv option:selected").text()) + parseFloat(1),
           // Filtro_TasaIgvDscto: tasaigv,
            Filtro_TasaIgv: tasaigv_maestro,
            Filtro_TasaIgvDscto: tasaigv_maestro,
            Filtro_CodigoTemporal: $('#hfCodigoTemporal').val(),
            Filtro_XmlDetalle: Sys.Serialization.JavaScriptSerializer.serialize(arrDetalle),
            Filtro_FlagIgv: 1
        ...
        MostrarEspera(true);
        F_AgregarTemporal_NV_NET(arg, function (result) {
            var str_resultado_operacion = "";
            var str_mensaje_operacion = "";
            ...
            if (result.split('~')[2] == "Los Producto(s) se han agregado con exito") {
                ...
                $('#MainContent_txtIgv').val(parseFloat(result.split('~')[6]).toFixed(2));
                $('#MainContent_txtSubTotal').val(parseFloat(result.split('~')[7]).toFixed(2));  
                 $('#MainContent_txtExonerada').val(parseFloat(result.split('~')[10]).toFixed(2)); 
                F_Update_Division_HTML('div_grvDetalleArticulo', result.split('~')[4]);
                $('#lblCantidadRegistro').text(F_Numerar_Grilla("grvDetalleArticulo", "lblimporte"));
                $('.ccsestilo').css('background', '#FFFFE0');
                $('#hfCodCtaCteNP').val(0);
        ...
        var tasaigv = parseFloat($("#MainContent_ddlIgv option:selected").text()) + parseFloat(1);
                ...
                F_AgregarTemporal_NET(arg, function (result) {
                    ...
                    if (result.split('~')[2] == "Los Producto(s) se han agregado con exito") {                        
                        ...
                        $('#MainContent_txtPlaca1').val(data.Placa);
...
function F_ActualizarDescripcion(Fila) {
    try {
        ...
        var hfCategoriaIgv = txtDescripcion.replace('txtDescripcion', 'hfCodCategoriaIgvGR');
        var tasaigv = 1;
        var FlagIgv = 0;
            ...
            F_ReemplazarFactura_NET(arg, function (result) {
                ...
                if (str_resultado_operacion == "1") {
                   ...
                   F_CambioSerie_TipoDoc();
                    ...
                    $('#MainContent_txtPlaca1').val(result.split('~')[21]);
                    ...
                    if (result.split('~')[33]==0)
                        ...
                        $('#MainContent_chkConCodigo').prop('checked',true);
                    $('#MainContent_txtKM').val(result.split('~')[34]);
                    $('#MainContent_txtNroOC').val(result.split('~')[35]);
                    $('.ccsestilo').css('background', '#FFFFE0');
...
//            Cadena=Cadena + '<p></p>' + 'NUMERO OPERACION';
        ...
        if (Cadena != 'Ingresar los sgtes. Datos:')
           {   
               alertify.error(Cadena.toUpperCase());
               return false;
           }  
...
function F_Inicializar_TextBox()
{
    ...
    $('#MainContent_TextBox3').css('background', '#FFFFE0');
    $('#MainContent_TextBox4').css('background', '#FFFFE0');
...
function F_Chequear_NotaVenta_Todo(objRef) {
    var checkallid = '#' + objRef.id;
    var txtCantidadEntregada = '';
    var ddlLista_grilla = '';
    var txtcant_grilla = '';
    ...
    else
    {
     ...
     $('#MainContent_grvDetalleNV .chkDelete :checkbox').each(function () {
            chkGrillaNV = '#' + this.id;
            txtCantidadEntregada = chkGrillaNV.replace('chkEliminar', 'txtCantidadEntregada');
            lblCantidad = chkGrillaNV.replace('chkEliminar', 'lblCantidad');
            
            if ($(chkGrillaNV).is(':checked'))
            {
               $(txtCantidadEntregada).prop('disabled', false);
                $(txtCantidadEntregada).val($(lblCantidad).text());
                $(txtCantidadEntregada).focus();
...
function F_PermisoObservacion(Fila) {
    try {
        ...
        var grvNombre = 'MainContent_grvConsulta_grvPermisoObservacion_' + Col;
                var CodTipoDoc = $('#' + Fila.replace('pnlPermisoObservacion', 'hfCodTipoDoc')).val();
...
//                            }
                          ...
                          if (CodDirecionDestinoEdicion>0)
                              BuscarDireccionCotizacion('#hfCodigoTemporalEdicion',CodDirecionDestinoEdicion,'#MainContent_ddldireccionNuevaDestinoEdicion','',Codtipodoc);
                          if (CodDirecionTransportistaEdicion>0)
                              BuscarDireccionCotizacion('#hfCodigoTemporalEdicion',CodDirecionTransportistaEdicion,'#MainContent_ddldireccionNuevaTransportistaEdicion','',Codtipodoc);
...
function F_InicializarCajaTexto()
{
    ...
    $('#MainContent_txtPlaca1').css('background', '#FFFFE0');
    ...
    $("#MainContent_txtMotorSA").css('background', '#FFFFE0');
    $("#MainContent_txtMarcaMotor").css('background', '#FFFFE0');
    $("#MainContent_txtSerieMotor").css('background', '#FFFFE0');
    $("#MainContent_txtCilindrado").css('background', '#FFFFE0');
    $("#MainContent_txtCilindros").css('background', '#FFFFE0');
    $("#MainContent_txtCombustible").css('background', '#FFFFE0');
    $("#MainContent_txtPotencia").css('background', '#FFFFE0');
        
    $("#MainContent_txtAdicional1").css('background', '#FFFFE0');
    
    $("#MainContent_txtAdicional2").css('background', '#FFFFE0');
    
    $("#MainContent_txtAdicional3").css('background', '#FFFFE0');
    
    $("#MainContent_txtAdicional4").css('background', '#FFFFE0');
    
    $("#MainContent_txtAdicional5").css('background', '#FFFFE0');
...
function F_AgregarMotor(Fila) {
   
    try {
        var Contenedor = '#MainContent_';

        F_LimpiarCamposMotor();

           var imgID = Fila.id;

        var hfCodDetalleMotor = '#' + Fila.replace('lblimporte', 'hfCodDetalle');
        var txtDescripcion = '#' + Fila.replace('lblimporte', 'txtDescripcion');

        $('#hfCodDetalleMotor').val($(hfCodDetalleMotor).val());
        $('#DescripcionMotor').val($(txtDescripcion).val());
         $('#hfMotorEdicion').val(0);
        F_EditarCamposMotor();

        $("#div_EditarMotor").dialog({
            resizable: false,
            modal: true,
            title: "Edicion Motor",
            title_html: true,
...
function F_EditarMotor() {
    var arrAlmacen = new Array();
    if (!F_SesionRedireccionar(AppSession)) return false;
    
    var DocumentoventaCab = {};
    DocumentoventaCab["hfCodDetalleMotor"] = $('#hfCodDetalleMotor').val();
    DocumentoventaCab["hfMotorEdicion"] = $('#hfMotorEdicion').val();
    DocumentoventaCab["MotorSA"] = $('#MainContent_txtMotorSA').val();
    DocumentoventaCab["MarcaMotor"] = $('#MainContent_txtMarcaMotor').val();
    DocumentoventaCab["SerieMotor"] = $('#MainContent_txtSerieMotor').val();
    DocumentoventaCab["Cilindrado"] = $('#MainContent_txtCilindrado').val();
    DocumentoventaCab["Cilindros"] = $('#MainContent_txtCilindros').val();
    DocumentoventaCab["Combustible"] = $('#MainContent_txtCombustible').val();
    DocumentoventaCab["Potencia"] = $('#MainContent_txtPotencia').val();
    //
    
    DocumentoventaCab["Adicional1"] = $('#MainContent_txtAdicional1').val();
    DocumentoventaCab["Adicional2"] = $('#MainContent_txtAdicional2').val();
    DocumentoventaCab["Adicional3"] = $('#MainContent_txtAdicional3').val();
    DocumentoventaCab["Adicional4"] = $('#MainContent_txtAdicional4').val();
    DocumentoventaCab["Adicional5"] = $('#MainContent_txtAdicional5').val();
   
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: 'RegistroFacturaMultiple.aspx/F_Modificar_Motor_NET',
        data: JSON.stringify(DocumentoventaCab),
        dataType: "json",
        async: false,
        success: function (dbObject) {
            var data = dbObject.d;
            try {
                if (data.MsgError === "") {
                    ...
                    if( $('#hfMotorEdicion').val()==0)
                    {
                    F_ActualizarSoloDetalle();
                    }
                    else
                    {
                    F_LlenarGridDetalleEdicion(txtComision,0);  
                    }

                    F_LimpiarCamposMotor();
                    $('#div_EditarMotor').dialog('close');
                } else {
...
function F_LimpiarCamposMotor() {
if (!F_SesionRedireccionar(AppSession)) return false;
   

    //Valores por Defecto
    $('#hfCodDetalleMotor').val('0');
    $('#MainContent_txtMotorSA').val('');
    $('#MainContent_txtMarcaMotor').val('');
    $('#MainContent_txtSerieMotor').val('');
    $('#MainContent_txtCilindrado').val('');
    $('#MainContent_txtCilindros').val('');
    $('#MainContent_txtCombustible').val('');
    $('#MainContent_txtPotencia').val('');

}

function F_ActualizarSoloDetalle(){
  try 
        {
        if($('#MainContent_txtTotal').val()=='0.00')
        return;
        ...
        if ($('#MainContent_chkConIgvMaestro').is(':checked')) { 
             ...
             FlagIgv = 1;         
                ...
                MostrarEspera(true);
                F_ActualizarSoloDetalle_NET(arg, function (result) {
...
function F_EditarCamposMotor() {
   
 try 
        {
             if ($('#DescripcionMotor').val().indexOf('|')!= -1) {var MotorSemiA=$('#DescripcionMotor').val().split(':')[1].split('|')[0].trim() }else{MotorSemiA=''};
             if ($('#DescripcionMotor').val().indexOf('|')!= -1) {var MarcaMotor=$('#DescripcionMotor').val().split(':')[2].split('|')[0].trim() }else{MarcaMotor=''};
             if ($('#DescripcionMotor').val().indexOf('|')!= -1) {var SerieMotor=$('#DescripcionMotor').val().split(':')[3].split('|')[0].trim() }else{SerieMotor=''};
             if ($('#DescripcionMotor').val().indexOf('|')!= -1) {var Cilindrada=$('#DescripcionMotor').val().split(':')[4].split('|')[0].trim() }else{Cilindrada=''};
             if ($('#DescripcionMotor').val().indexOf('|')!= -1) {var Cilindros=$('#DescripcionMotor').val().split(':')[5].split('|')[0].trim() }else{Cilindros=''};
             if ($('#DescripcionMotor').val().indexOf('|')!= -1) {var Combustible=$('#DescripcionMotor').val().split(':')[6].split('|')[0].trim() }else{Combustible=''};
             if ($('#DescripcionMotor').val().indexOf('|')!= -1) {var Potencia=$('#DescripcionMotor').val().split(':')[7].split('|')[0].trim() }else{Potencia=''};


             $('#MainContent_txtMotorSA').val(MotorSemiA);
             $('#MainContent_txtMarcaMotor').val(MarcaMotor);
             $('#MainContent_txtSerieMotor').val(SerieMotor);
             $('#MainContent_txtCilindrado').val(Cilindrada);
             $('#MainContent_txtCilindros').val(Cilindros);
             $('#MainContent_txtCombustible').val(Combustible);
             $('#MainContent_txtPotencia').val(Potencia);
...
function F_AgregarMotor_Edicion(Fila) {
    ...
    try {
        ...
        var hfCodDetalleMotor = '#' + Fila.replace('lblimporte', 'hfID');
        var lblDescripcion = '#' + Fila.replace('lblimporte', 'lblDescripcion');
       txtComision=  Fila.replace('grvDetalleEdicion_lblimporte', 'grvConsulta_imgEditarDocumento');

        $('#hfCodDetalleMotor').val($(hfCodDetalleMotor).val());
        $('#DescripcionMotor').val($(lblDescripcion).text());
         $('#hfMotorEdicion').val(1);
        F_EditarCamposMotor();
