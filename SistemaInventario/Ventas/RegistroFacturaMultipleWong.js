var AppSession = "../Ventas/RegistroFacturaMultipleWong.aspx";
var Codtipodoc=1
var CodigoMenu = 4000;
var CodigoInterno = 2;
var CodCategoria = 1;
...
function F_AgregarArticulo(ControlID, DirectoBoton) {
    ...
    hfUbicacion = imgAgregar_grilla.replace('imgAgregar', 'hfUbicacion');
    ...
    $('#MainContent_txtUbicacion').val($(hfUbicacion).val());
            ...
            // FIN DATOS DE LA GUIA         
                ...
                if (str_resultado_operacion == "1") 
                {
                    if (str_mensaje_operacion=='Se Grabo Correctamente')
                    {
                        ...
                        if ($('#MainContent_chkWhatsApp').is(':checked') & F_WhatsAppHabilitado() == "1"){
...
var arrDetalle = new Array();
    ...
    var objParams = {
                      ...
                      Filtro_Observacion: $('#MainContent_txtObservacionFacturaEdicion').val(),
            ...
            try {
                ...
                $.each(data.rows, function (index, item) {
                    ...
                    var li = fmt2.replace('@Nro', Count).replace('@ID', 'COTIZACION_CODPROFORMA_' + Count).replace('@CodProforma', item.CodProforma)
                                        .replace('@NroDocumento', Number(item.Numero)).replace("@ElVendedor", item.Vendedor+"("+item.Total+")").replace('@Cliente', item.Cliente)
