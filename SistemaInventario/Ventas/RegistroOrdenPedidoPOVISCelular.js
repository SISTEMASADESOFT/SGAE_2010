var AppSession = "../Ventas/RegistroOrdenPedidoPOVISCelular.aspx";
...
$(document).ready(function () {
  ...
  if($("#MainContent_txtDescuento2").val()<100 && $("#MainContent_txtDescuento2").val()>=0){
    ...
    if (!F_SesionRedireccionar(AppSession)) return false;
    if (F_PermisoOpcion(CodigoMenu, 4001801, '') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informac
        try 
        {
...
function F_AgregarArticulo(ControlID, DirectoBoton) {
    ...
    if ($.trim($('#MainContent_txtNroRuc').val())=='')
        $('#hfCodCategoriaCliente').val(2);
    else
...
function F_GrabarDocumento(){
  try 
        ...
        var objParams = {
            ...
            Filtro_CodFormaPago: $(Contenedor + 'ddlFormaPago').val(),
            Filtro_CodOrdenPedidoAnterior: $('#hfCodFacturaAnterior').val(),            
            Filtro_CodEmpleado: $('#MainContent_ddlVendedor').val(),
            Filtro_Descuento: $(Contenedor + 'txtDescuentoTotal').val(), 
...
if (F_PermisoOpcion(CodigoMenu, CodigoInterno, 'Editar') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informacion
    ...
    if ($(lblEstado).text() != 'PREPARACION')
    {
    alertify.log('EL DOCUMENTO DEBE ESTAR PREPARACION');
    return false;
    }
            ...
            try {
                ...
                F_AgregarTemporal_NET(arg, function (result) {
                    ...
                    if (result.split('~')[2] == "Los Producto(s) se han agregado con exito") {
                        ...
                        $('#MainContent_txtDescuentoTotal').val(data.Descuento);
                        $('#MainContent_txtDescuentoTotal').val(data.Descuento);
...
if (F_PermisoOpcion(CodigoMenu, CodigoInterno, 'Eliminar') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informac
  var imgID = Fila.id;
        ...
        var TipoArchivo = 'application/pdf';
        var CodMenu = '215';
        var NombreArchivo = 'Web_Reporte_Ventas_rptOrdenPedido_Povis_A4.rpt';
...
function F_AnularPopUP(Fila) {
    ...
    if ($(lblEstado).text() == "ANULADO") {
            alertify.log("LA ORDEN DE PEDIDO SE ENCUENTRA ANULADA");
            return false;
        }
    ...
    if(!confirm("ESTA SEGURO DE ANULAR LA ORDEN DE PEDIDO : " + $(lblnumero_grilla).text() + "\n" + "DEL CLIENTE : " +  $(lblcliente_grilla).text()))
    return false;
...
//        }
        ...
        case "PREPARACION":
             if (!confirm("ESTA SEGURO DE PASAR A PENDIENTE LA ORDEN DE PEDIDO : " + $(lblNumero).text() + "\n" + "DEL CLIENTE : " + $(lblCliente).text()))
             return false;
        break;
        case "PENDIENTE":
            if (!confirm("ESTA SEGURO DE PASAR A PREPARACION LA ORDEN DE PEDIDO : " + $(lblNumero).text() + "\n" + "DEL CLIENTE : " + $(lblCliente).text()))
            return false;
        break;
        default:
           // if (!confirm("ESTA SEGURO DE ABRIR LA ORDEN DE PEDIDO : " + $(lblNumero).text() + "\n" + "DEL CLIENTE : " + $(lblCliente).text()))
        ...
        $.ajax({
            type: "POST",
            contentType: "application/json; charset=utf-8",
            url: '../Servicios/Servicios.asmx/F_Aprobar_OrdenPedido_Preparacion',
