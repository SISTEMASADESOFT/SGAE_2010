...
$(document).ready(function () {
    ...
    $('#MainContent_txtDesde').datepicker({beforeShowDay: function(date) {
      ...
      $('#MainContent_txtDesdeNP').datepicker({onSelect: function() {
      var date = $(this).datepicker('getDate');
      if (date) {
            date.setDate(1);
            $(this).datepicker('setDate', date);
    ...
    $('#MainContent_txtDesdeNP').datepicker({beforeShowDay: function(date) {
      return [date.getDate() == 1, ''];
    }});
            ...
            (
                ...
                function (result) {
                    ...
                    if (str_resultado_operacion == "1") 
                    {
                        ...
                        F_Update_Division_HTML('div_UsuarioNP', result.split('~')[17]);
                        F_Update_Division_HTML('div_SucursalNP', result.split('~')[18]);
                        GridArticulosInicializado = result.split('~')[15];
                        $('#MainContent_ddlMoneda').val(2);
                        $('#MainContent_ddlFormaPago').val(1);
                        $('#MainContent_ddlVendedor').val(result.split('~')[13]);
                        $('#MainContent_dllUsuarionNP').val(result.split('~')[9]);
...
if (F_PermisoOpcion(CodigoMenu, CodigoInterno, 'Editar') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informacion
    ...
    $.ajax({
        ...
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        url: 'RegistroFacturaMultipleReeim.aspx/F_ObtenerCotizacion_PorID_Net',
        dataType: "json",
        ...
        success: function (dbObject) {
            ...
            catch (x) { alertify.log("ERROR DETECTADO: " + x); }
            MostrarEspera(false);
        },
        error: function (xhr, ajaxOptions, thrownError) {
...
if (F_PermisoOpcion(CodigoMenu, 4000103, '') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informac
...
function imgMas_ClickDetalle(Control) {
    ...
    if (Src.indexOf('plus') >= 0) {
        ...
        var grid = $(Control).next();
        F_LlenarGridDetalleProducto(grid.attr('id'));
    }
    else {
        $(Control).attr("src", "../Asset/images/plus.gif");
        $(Control).closest("tr").next().remove();
...
function F_LlenarGridDetalleProducto(Fila) {
    try {
        var nmrow = 'MainContent_grvConsultaUltimosPrecios_pnlOrdersDetalle_0';
        var Col = Fila.split('_')[3];
        var lblCodigo = '#' + Fila.replace('pnlOrdersDetalle', 'hfcoddoc');
        var hfTipoDoc = '#' + Fila.replace('pnlOrdersDetalle', 'hfcodtipodoc');
        var grvNombre = 'MainContent_grvConsultaUltimosPrecios_grvDetalleproductos_' + Col;
        HfgvDetalle = '#' + Fila.replace('pnlOrdersDetalle', 'hfDetalleCargado');

        if ($(HfgvDetalle).val() === "1") {
            $(CtlgvDetalle).closest('tr').after('<tr><td></td><td colspan = "999">' + $(CtlgvDetalle).next().html() + '</td></tr>');
            $(CtlgvDetalle).attr('src', '../Asset/images/minus.gif');
        }
        else {
            {
                var objParams =
                        {
                            Filtro_Col: Col,
                            Filtro_Codigo: $(lblCodigo).val(),
                            Filtro_CodTipoDoc: $(hfTipoDoc).val(),
                            Filtro_grvNombre: grvNombre
                        };
                ...
                $(CtlgvDetalle).attr('src', '../Asset/images/loading.gif');
                F_LlenarGridDetalleProducto_NET(arg, function (result) {
