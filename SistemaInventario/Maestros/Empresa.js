...
if (F_PermisoOpcion(CodigoMenu, CodigoInterno, 'Editar') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informac
    try {
        ...
        CampoGrilla = '#' + imgID.replace('imgEditarRegistro', 'hfFlagBuenContribuyente'); CampoEdit = Cuerpo + 'chkBuenContribuyenteEdicion'; 
        if ($(CampoGrilla).val()==0)
            $(CampoEdit).prop("checked",false);
        else 
            $(CampoEdit).prop("checked",true);
...
function F_EdicionRegistro() {
        var IdImagen = 0;
  var Contenedor = '#MainContent_';
        var FlagBuenContribuyente = 0; if ($(Contenedor + 'chkBuenContribuyenteEdicion').is(':checked'))   FlagBuenContribuyente=1;
        try
        {
    ...
    try {      
        var objParams = {
            ...
            Filtro_FlagBuenContribuyente: FlagBuenContribuyente,
