var AppSession = "../Maestros/ProveedoresUnitarioTEK.aspx";
...
function F_ValidarGrabarDocumento() {
    ...
    try {
        ...
        if ($(Cuerpo + 'ddlTipoCliente').val() == '3' && $(Cuerpo + 'txtNroRuc').val() != '55555555555')
        {
           ...
           if ($(Cuerpo + 'ddlTipoCliente').val() == '4' )
        {
            if ($(Cuerpo + 'txtPlaca').val() == '')
            Cadena = Cadena + '<p></p>' + 'Placa';

            if ($(Cuerpo + 'txtLicencia').val() == '')
            Cadena = Cadena + '<p></p>' + 'Licencia';

            if (!($(Cuerpo + 'txtPlaca').val().length >= 6 && $(Cuerpo + 'txtPlaca').val().length <= 7)) {
                Cadena = Cadena + '<p></p>' + 'Placa Invalida';
            }

            if (!($(Cuerpo + 'txtLicencia').val().length >= 9 && $(Cuerpo + 'txtLicencia').val().length <= 10)) {
                Cadena = Cadena + '<p></p>' + 'Licencia Invalida';
            }
        }
...
function F_EditarRegistro(Fila) {
    ...
    try {
        ...
        var hfFlagRelacionado = '#' + imgID.replace('imgEditarRegistro', 'hfFlagRelacionado');
        var Cuerpo = '#MainContent_';

        $(Cuerpo + 'txtRucEdicion').val($(hfNroRuc_grilla).val());
        $(Cuerpo + 'chkRelacionado').prop('checked', $(hfFlagRelacionado).val() === '1');
...
function F_EdicionRegistro() {
    ...
    try {
        ...
        if ($('#MainContent_chkFlagBloqueoCreditoEdicion').is(':checked'))
         ...
         if ($('#MainContent_chkRelacionado').is(':checked'))
            FlagRelacionado = 1;
        ...
        var objParams = {
            ...
            Filtro_FlagRelacionado: FlagRelacionado,
        };
...
function F_ValidarEdicionDocumento() {
    try {
        ...
        if ($(Cuerpo + 'txtDniEdicion').val() != '' && $(Cuerpo + 'txtDniEdicion').val().length < 8 & $(Cuerpo + 'ddlTipoCliente_Edicion').val() != '3')
           ...
           if ($(Cuerpo + 'ddlTipoClienteEdicion').val() == '4' )
        {
            if ($(Cuerpo + 'txtPlacaEdicion').val() == '')
            Cadena = Cadena + '<p></p>' + 'Placa';

            if ($(Cuerpo + 'txtLicenciaEdicion').val() == '')
            Cadena = Cadena + '<p></p>' + 'Licencia';

            if (!($(Cuerpo + 'txtPlacaEdicion').val().length >= 6 && $(Cuerpo + 'txtPlacaEdicion').val().length <= 7)) {
                Cadena = Cadena + '<p></p>' + 'Placa Invalida';
            }

            if (!($(Cuerpo + 'txtLicenciaEdicion').val().length >= 9 && $(Cuerpo + 'txtLicenciaEdicion').val().length <= 10)) {
                Cadena = Cadena + '<p></p>' + 'Licencia Invalida';
            }
        }
