...
namespace CapaNegocios
{
    ...
    public class OrdenPedidoCabCN
    {
            ...
            try
            {
                ...
                try
                {
                    foreach (DataRow i in dtDatos.Rows)
                    {
                        ...
                        objEntidadBE.NombreComercial = i["NombreComercial"].ToString();
                        objEntidadBE.CodCategoria = int.Parse(i["CodCategoria"].ToString());
                        objEntidadBE.Descuento = decimal.Parse(i["Descuento"].ToString());
                        objEntidadBE.lOrdenPedidoDet = new List<OrdenPedidoDetCE>();
                        dtDatos = (new OrdenPedidoCabCN()).F_OrdenPedidoDet_ListarXCodigo(objEntidadBE);
                        foreach (DataRow r in dtDatos.Rows)
                        {
        ...
        public OrdenPedidoCabCE F_OrdenPedido_Aprobacion_Preparacion(int CodOrdenPedido, string Observacion, int CodUsuario)
        {
            try
            {
                return obj.F_OrdenPedido_Aprobacion_Preparacion(CodOrdenPedido, Observacion, CodUsuario);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public OrdenPedidoCabCE F_OrdenPedidoCab_ObtenerXNumero_Povis(OrdenPedidoCabCE objEntidadBE)
        {

            try
            {
                DataTable dtDatos = obj.F_OrdenPedidoCab_ObtenerXNumero_Povis(objEntidadBE);
                try
                {
                    foreach (DataRow i in dtDatos.Rows)
                    {
                        ...
                        objEntidadBE.CodTipoDoc = int.Parse(i["CodTipoDoc"].ToString());
                        objEntidadBE.lOrdenPedidoDet = new List<OrdenPedidoDetCE>();
                        dtDatos = (new OrdenPedidoCabCN()).F_OrdenPedidoDet_ListarXCodigo_Povis(objEntidadBE);
                        foreach (DataRow r in dtDatos.Rows)
                        {
                            objEntidadBE.lOrdenPedidoDet.Add(new OrdenPedidoDetCE()
                            {
        ...
        public DataTable F_OrdenPedidoDet_ListarXCodigo_Povis(OrdenPedidoCabCE objEntidadBE)
        {

            try
            {

                return obj.F_OrdenPedidoDet_ListarXCodigo_Povis(objEntidadBE);
